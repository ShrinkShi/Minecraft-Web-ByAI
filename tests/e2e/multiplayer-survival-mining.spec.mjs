...
async function setViewAndLock(page,yaw,pitch=0){
  const canvas=page.locator('#game-canvas');await canvas.click({position:{x:8,y:8}});await expect.poll(()=>page.evaluate(()=>document.pointerLockElement?.id||null),{timeout:5_000}).toBe('game-canvas');await page.evaluate(({yaw,pitch})=>globalThis.__minecraftE2E?.setLook(yaw,pitch),{yaw,pitch});await page.waitForTimeout(150);
}
...
test('holding primary mines survival blocks on the server and the drop returns through authoritative inventory',async({page})=>{
  ...
  try{
    ...
    const [session]=[...runtime.authoritative.sessions];expect(session).toBeTruthy();await setViewAndLock(page,0,0);const player=runtime.authoritative.snapshot(session),x=Math.floor(player.position.x),y=Math.floor(player.position.y+PLAYER_EYE_HEIGHT),z=Math.floor(player.position.z)-1;const change=runtime.setBlock(x,y,z,BLOCK.DIRT);expect(change.changed).toBe(true);await expect.poll(()=>page.evaluate(({x,y,z})=>globalThis.__minecraftE2E?.worldBlock(x,y,z),{x,y,z}),{timeout:5_000}).toBe(BLOCK.DIRT);
    const canvas=page.locator('#game-canvas'),box=await canvas.boundingBox();expect(box).toBeTruthy();await page.mouse.move(box.x+box.width/2,box.y+box.height/2);await page.mouse.down({button:'left'});await page.waitForTimeout(120);await expect(page.locator('#toast')).not.toContainText('联机方块/战斗权威尚未接入');await expect.poll(()=>runtime.survivalBreak.progress(session)?.progress||0,{timeout:2_000}).toBeGreaterThan(0);await expect.poll(()=>runtime.world.getBlock(x,y,z),{timeout:3_000}).toBe(BLOCK.AIR);await page.mouse.up({button:'left'});
