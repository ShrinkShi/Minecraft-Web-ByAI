...
test('holding primary renders server mining progress, breaks the block, and returns the drop through authoritative inventory',async({page})=>{
  ...
  try{
    ...
    const player=runtime.authoritative.snapshot(session),x=Math.floor(player.position.x),y=Math.floor(player.position.y+PLAYER_EYE_HEIGHT),z=Math.floor(player.position.z)-1;runtime.setBlock(x,y,z,BLOCK.AIR);const change=runtime.setBlock(x,y,z,BLOCK.LOG);expect(change.changed).toBe(true);await expect.poll(()=>page.evaluate(({x,y,z})=>globalThis.__minecraftE2E?.worldBlock(x,y,z),{x,y,z}),{timeout:5_000}).toBe(BLOCK.LOG);
    const breakMeter=page.locator('#break-meter');await expect(breakMeter).toHaveClass(/hidden/);
    await page.mouse.down({button:'left'});await expect.poll(()=>runtime.server.getSessionInputState(session)?.control?.primary,{timeout:5_000}).toBe(true);await page.waitForTimeout(120);await expect(page.locator('#toast')).not.toContainText('联机方块/战斗权威尚未接入');
    await expect.poll(async()=>{const serverProgress=runtime.survivalBreak.progress(session)?.progress||0,visual=await page.evaluate(()=>{const meter=document.querySelector('#break-meter'),fill=meter?.querySelector('span');return{visible:!!meter&&!meter.classList.contains('hidden'),width:parseFloat(fill?.style.width||'0')};});return serverProgress>0&&visual.visible&&visual.width>0;},{timeout:2_000,message:'active authoritative mining progress should be rendered by the browser in the same observation window'}).toBe(true);
    await expect.poll(()=>runtime.world.getBlock(x,y,z),{timeout:4_000}).toBe(BLOCK.AIR);await page.mouse.up({button:'left'});await expect.poll(()=>runtime.server.getSessionInputState(session)?.control?.primary,{timeout:5_000}).toBe(false);await expect(breakMeter).toHaveClass(/hidden/,{timeout:2_000});await expect.poll(()=>page.evaluate(()=>parseFloat(document.querySelector('#break-meter span')?.style.width||'0')),{timeout:2_000}).toBe(0);
