...
test('holding primary mines survival blocks on the server and the drop returns through authoritative inventory',async({page})=>{
  ...
  try{
    ...
    await page.mouse.down({button:'left'});await expect.poll(()=>runtime.server.getSessionInputState(session)?.control?.primary,{timeout:5_000}).toBe(true);await page.waitForTimeout(120);await expect(page.locator('#toast')).not.toContainText('联机方块/战斗权威尚未接入');await expect.poll(()=>runtime.survivalBreak.progress(session)?.progress||0,{timeout:2_000}).toBeGreaterThan(0);await expect.poll(()=>runtime.world.getBlock(x,y,z),{timeout:4_000}).toBe(BLOCK.AIR);await page.mouse.up({button:'left'});await expect.poll(()=>runtime.server.getSessionInputState(session)?.control?.primary,{timeout:5_000}).toBe(false);
