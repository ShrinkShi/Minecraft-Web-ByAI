...
test('imported Minecraft runtime assets resolve and decode through the real HTTP server',async({page,request})=>{
  ...
  expect(modelManifest).toMatchObject({sourceKind:'directory',sourceRoot:'MC原版素材assets',closure:{blockstates:24,models:70,textures:39,metadata:0}});
  expect(modelManifest.atlas).toMatchObject({path:'model-texture-atlas.png',width:128,height:128,gutterPx:1,sha256:'28dea729513157f790032964dc4607a88ba6657e72d3e9eca5a9cc85fa5ce1b5'});
  expect(Object.keys(modelManifest.textures)).toHaveLength(39);expect(modelManifest.textures['minecraft:block/grass']).toBeTruthy();expect(modelManifest.textures['minecraft:block/glass']).toBeTruthy();expect(modelManifest.textures['minecraft:block/farmland']).toBeTruthy();expect(modelManifest.textures['minecraft:block/farmland_moist']).toBeTruthy();expect(modelManifest.textures['minecraft:block/wheat_stage7']).toBeTruthy();expect(modelManifest.textures['minecraft:block/oak_planks']).toBeTruthy();expect(modelManifest.textures['minecraft:block/granite']).toBeTruthy();expect(modelManifest.textures['minecraft:block/cherry_planks']).toBeTruthy();
  for(const key of ['minecraft:block/furnace_front','minecraft:block/furnace_front_on','minecraft:block/furnace_side','minecraft:block/furnace_top','minecraft:block/oak_planks','minecraft:block/granite','minecraft:block/cherry_planks'])expect(modelManifest.textures[key]?.source).toContain('MC原版素材assets/minecraft/textures/block/');
