...
test('imported Minecraft runtime assets resolve and decode through the real HTTP server',async({page,request})=>{
  ...
  expect(modelManifest).toMatchObject({sourceKind:'directory',sourceRoot:'MC原版素材assets',closure:{blockstates:13,models:60,textures:29,metadata:0}});
  expect(modelManifest.atlas).toMatchObject({path:'model-texture-atlas.png',width:128,height:128,gutterPx:1,sha256:'d3770949b24633637dd68c7a96a92217b4d46265b5bbef3ea965831eeb04fd31'});
  expect(Object.keys(modelManifest.textures)).toHaveLength(29);expect(modelManifest.textures['minecraft:block/short_grass']).toBeTruthy();expect(modelManifest.textures['minecraft:block/glass']).toBeTruthy();expect(modelManifest.textures['minecraft:block/farmland']).toBeTruthy();expect(modelManifest.textures['minecraft:block/farmland_moist']).toBeTruthy();expect(modelManifest.textures['minecraft:block/wheat_stage7']).toBeTruthy();
