...
test('multiplayer inventory slot clicks round-trip through server authority while equipment stays fail-closed',async({page})=>{
  ...
  try{
    ...
    const inventoryPanel=page.locator('#inventory');await page.keyboard.press('KeyE');await expect(inventoryPanel).not.toHaveClass(/hidden/);const hotbarSlot=page.locator(`#inventory [data-inv-index="${HOTBAR_START}"]`);await hotbarSlot.click({force:true});
    ...
    const revisionBeforeBlockedClick=runtime.inventories.snapshot(session).revision;await page.locator('#inventory [data-equipment-slot="head"]').click({force:true});await expect(page.locator('#toast')).toContainText('联机装备事务尚未服务端化');await page.waitForTimeout(150);expect(runtime.inventories.snapshot(session).revision).toBe(revisionBeforeBlockedClick);expect(runtime.inventories.snapshot(session).cursor).toEqual({id:'block:1',count:64});
