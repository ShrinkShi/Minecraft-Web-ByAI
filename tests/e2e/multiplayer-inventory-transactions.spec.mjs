...
test('multiplayer inventory slot clicks round-trip through server authority while equipment stays fail-closed',async({page})=>{
  ...
  try{
    ...
    await page.locator('#inventory').evaluate(element=>element.classList.remove('hidden'));const hotbarSlot=page.locator(`#inventory [data-inv-index="${HOTBAR_START}"]`);await hotbarSlot.click({force:true});
