...
test('Creative inventory uses vanilla-style category and survival inventory tabs with 3D block icons',async({page})=>{
  ...
  await page.getByRole('button',{name:'生存物品栏'}).click();await expect(inventory).toHaveAttribute('data-creative-view','survival');await expect(page.getByRole('button',{name:'生存物品栏'})).toHaveClass(/active/);await expect(body).toBeHidden();await expect(panel).toHaveCSS('height','350px');await expect(page.locator('#inventory .inventory-top')).not.toHaveClass(/hidden/);await expect(page.locator('#inventory .player-preview')).toBeVisible();await expect(page.locator('#equipment-slots')).toBeVisible();await expect(page.locator('#inventory-grid')).toBeVisible();await expect(page.locator('#inventory-hotbar')).toBeVisible();await expect(page.locator('#inventory .crafting-area')).toBeHidden();
