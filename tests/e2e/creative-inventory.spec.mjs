...
test('Creative inventory uses vanilla-style category and survival inventory tabs with 3D block icons',async({page})=>{
  ...
  await key(page,'KeyE');
  const inventory=page.locator('#inventory'),panel=inventory.locator('.inventory-panel'),catalog=page.locator('[data-creative-catalog]'),body=page.locator('.creative-catalog-body'),search=page.locator('.creative-catalog-search'),grid=page.locator('.creative-catalog-grid');
  ...
  await expect(page.locator('[data-creative-category]')).toHaveCount(8);await expect(page.locator('[data-creative-view="survival"]')).toHaveCount(1);await expect(page.locator('[data-creative-category="building"]')).toHaveClass(/active/);await expect(page.locator('.creative-catalog-section-title')).toHaveText('建筑方块');
  ...
  const wool=grid.locator('[data-creative-item="white_wool"]');await expect(wool).toBeVisible();const woolCanvas=wool.locator('.block-item-canvas');await expect(woolCanvas).toHaveCount(1);await expect(woolCanvas).toHaveAttribute('data-render-state','ready');
