...
test('tracked Java 1.20.1 model runtime reaches mesh Worker and VoxelWorld with shared model-atlas materials',async({page})=>{
  ...
  const result=await page.evaluate(async()=>{
    ...
    const {VoxelWorld}=worldModule;
    const waitFor=async(predicate,{timeout=10_000,label='condition'}={})=>{
      const started=performance.now();
      while(performance.now()-started<timeout){const value=predicate();if(value)return value;await new Promise(resolve=>setTimeout(resolve,20));}
    ...
    const nextMessage=(worker,expected,timeout=10_000)=>new Promise((resolve,reject)=>{
      ...
      const onMessage=event=>{
        ...
        if(message?.type==='minecraft-model-runtime-error'){cleanup();reject(new Error(message.message));return;}
    ...
    data[index(3,20,4)]=BLOCK.CRAFTING_TABLE;
    ...
    const directWorker={
      ...
      textureCount:ready.textureCount,
      legacyOpaqueEmpty:message.opaque.empty,
      waterEmpty:message.water.empty,
      specials:message.specials.length,
      opaqueFaces:message.interpreted.opaque.faceCount,
      opaqueVertices:message.interpreted.opaque.vertexCount,
      opaqueIndexBytes:message.interpreted.opaque.indices?.byteLength??0,
      cutoutEmpty:message.interpreted.cutout.empty,
      translucentEmpty:message.interpreted.translucent.empty
    };

    const rendererScene={children:[],add(object){this.children.push(object);},remove(object){this.children=this.children.filter(entry=>entry!==object);}};
    const renderer=new MinecraftModelWorldRenderer(rendererScene);
    const signal=renderer.handleWorkerMessage(ready);
    const rendered=renderer.makeChunkMeshes(message.interpreted,0,0);
    let geometryDisposeCount=0,textureDisposeCount=0,materialDisposeCount=0;
    rendered.opaque.geometry.addEventListener('dispose',()=>geometryDisposeCount++);
    renderer.atlasTexture.addEventListener('dispose',()=>textureDisposeCount++);
    renderer.materials.opaque.addEventListener('dispose',()=>materialDisposeCount++);
    const rendererState={
      signal,
      status:renderer.status,
      children:rendererScene.children.length,
      name:rendered.opaque.name,
      indexCount:rendered.opaque.geometry.index.count,
      normalType:rendered.opaque.geometry.getAttribute('normal').array.constructor.name,
      colorType:rendered.opaque.geometry.getAttribute('color').array.constructor.name,
      sharedMaterial:rendered.opaque.material===renderer.materials.opaque,
      assetKey:rendered.opaque.material.map?.userData?.assetKey||null
    };
    renderer.disposeChunkMeshes(rendered);
    const childrenAfterChunkDispose=rendererScene.children.length;
    renderer.dispose();
    const rendererDisposed={geometryDisposeCount,textureDisposeCount,materialDisposeCount};

    const worldScene={children:[],add(object){this.children.push(object);},remove(object){this.children=this.children.filter(entry=>entry!==object);}};
    const world=new VoxelWorld(worldScene,{seed:'interpreted-model-runtime',prompt:'平原',renderDistance:0});
    await world.generateArea(0,0);
    await waitFor(()=>world.minecraftModelRenderer.status!=='loading'&&world.meshWorkerReady,{label:'VoxelWorld model-runtime initialization'});
    ...
    const worldState={
      status:world.minecraftModelRenderer.status,
      blockIds:[...world.minecraftModelRenderer.blockIds],
      textureCount:world.minecraftModelRenderer.textureCount,
      modelName:worldRecord.interpreted.opaque.name,
      indexCount:worldRecord.interpreted.opaque.geometry.index.count,
      sharedMaterial:worldRecord.interpreted.opaque.material===world.minecraftModelRenderer.materials.opaque,
      assetKey:worldRecord.interpreted.opaque.material.map?.userData?.assetKey||null,
      hasLegacyTerrain:!!worldRecord.opaque
    };
    world.dispose();
    const worldDisposed={children:worldScene.children.length,worldModelGeometryDisposed,worldModelTextureDisposed};
