...
test('multiplayer ordinary chat is displayed only after authoritative server broadcast',async({page})=>{
  ...
  try{
    ...
    const wrap=page.locator('#chat-input-wrap'),input=page.locator('#chat-input'),log=page.locator('#chat-log');await page.keyboard.press('t');await expect(wrap).not.toHaveClass(/hidden/);await expect(input).toBeFocused();await input.fill('服务器权威聊天测试');await page.keyboard.press('Enter');
