...
test('multiplayer equipment exchanges atomically with the authoritative inventory cursor',async({page})=>{
  ...
  try{
    ...
    await expect(page.locator('#loading')).toHaveClass(/hidden/,{timeout:30_000});await expect.poll(()=>page.evaluate(()=>globalThis.__minecraftE2E?.multiplayer?.()),{timeout:30_000}).toMatchObject({sessionKind:'multiplayer',state:'ready',ready:true,worldId:'e2e-equipment-transactions'});await expect(page.locator('#chat-log')).toContainText('背包、装备、玩家 2×2 合成与工作台 3×3 容器');
    const [session]=[...runtime.authoritative.sessions];expect(session).toBeTruthy();await expect.poll(()=>page.evaluate(()=>globalThis.__minecraftE2E?.equipmentRevision?.()),{timeout:5_000}).toBe(0);await expect.poll(()=>page.evaluate(()=>globalThis.__minecraftE2E?.equipmentSlot?.('head')),{timeout:5_000}).toBe(null);await expect(page.locator('#armor-row .armor-icon.half')).toHaveCount(0);
    ...
    await headSlot.click({force:true});await expect.poll(()=>runtime.equipments.snapshot(session).revision,{timeout:5_000}).toBe(1);expect(runtime.inventories.snapshot(session).revision).toBe(3);expect(runtime.inventories.snapshot(session).cursor).toBe(null);expect(runtime.equipments.snapshot(session).slots.head).toEqual({id:'leather_helmet',count:1});expect(runtime.equipments.armorPoints(session)).toBe(1);await expect.poll(()=>page.evaluate(()=>globalThis.__minecraftE2E?.equipmentRevision?.()),{timeout:5_000}).toBe(1);await expect.poll(()=>page.evaluate(()=>globalThis.__minecraftE2E?.equipmentSlot?.('head')),{timeout:5_000}).toEqual({id:'leather_helmet',count:1});await expect.poll(()=>page.evaluate(()=>globalThis.__minecraftE2E?.inventoryCursor?.()),{timeout:5_000}).toBe(null);await expect(headSlot).toHaveAttribute('aria-label',/皮革帽子/);await expect(page.locator('#armor-row .armor-icon.half')).toHaveCount(1);

    await headSlot.click({force:true});await expect.poll(()=>runtime.equipments.snapshot(session).revision,{timeout:5_000}).toBe(2);expect(runtime.inventories.snapshot(session).revision).toBe(4);expect(runtime.equipments.snapshot(session).slots.head).toBe(null);expect(runtime.inventories.snapshot(session).cursor).toEqual({id:'leather_helmet',count:1});await expect.poll(()=>page.evaluate(()=>globalThis.__minecraftE2E?.equipmentSlot?.('head')),{timeout:5_000}).toBe(null);await expect.poll(()=>page.evaluate(()=>globalThis.__minecraftE2E?.inventoryCursor?.()),{timeout:5_000}).toEqual({id:'leather_helmet',count:1});await expect(page.locator('#armor-row .armor-icon.half')).toHaveCount(0);expect(errors).toEqual([]);
