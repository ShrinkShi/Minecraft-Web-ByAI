...
async function lockPointerAndSetView(page,yaw,pitch=0){
...
test('creative browser secondary waits for authoritative tick before placing a block',async({page})=>{
  ...
  try{
    ...
    const [session]=[...runtime.authoritative.sessions];expect(session).toBeTruthy();const beforeLockSequence=runtime.server.getSessionInputState(session)?.control?.sequence??-1;
    await lockPointerAndSetView(page,0,0);
    await expect.poll(()=>runtime.server.getSessionInputState(session)?.control?.sequence??-1,{timeout:5_000}).toBeGreaterThan(beforeLockSequence);
    await expect.poll(()=>runtime.server.getSessionInputState(session)?.control?.primary,{timeout:5_000}).toBe(false);
    tick();
    await expect.poll(()=>runtime.authoritative.snapshot(session)?.tick,{timeout:5_000}).toBe(1);

    const snapshot=runtime.authoritative.snapshot(session),position=snapshot.position,x=Math.floor(position.x),y=Math.floor(position.y+PLAYER_EYE_HEIGHT),anchorZ=Math.floor(position.z)-2,placeZ=anchorZ+1;
