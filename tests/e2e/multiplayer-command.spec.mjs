...
test('multiplayer slash command mutates inventory only through authoritative server result',async({page})=>{
  ...
  try{
    ...
    const wrap=page.locator('#chat-input-wrap'),input=page.locator('#chat-input');await wrap.evaluate(element=>element.classList.remove('hidden'));await input.focus();await input.fill('/give stick 3');await page.keyboard.press('Enter');
