...
async function lockAndAimAtBlock(page){
  const canvas=page.locator('#game-canvas');await canvas.click({position:{x:8,y:8}});await expect.poll(()=>page.evaluate(()=>document.pointerLockElement?.id||null),{timeout:5_000}).toBe('game-canvas');
  const candidates=[];for(const pitch of [-.25,-.45,-.65,-.85])for(const yaw of [0,Math.PI/2,Math.PI,-Math.PI/2])candidates.push([yaw,pitch]);
  for(const [yaw,pitch] of candidates){await page.evaluate(({yaw,pitch})=>globalThis.__minecraftE2E?.setLook(yaw,pitch),{yaw,pitch});await page.waitForTimeout(60);if(/Aim\s+-?\d+\/-?\d+\/-?\d+/.test(await page.locator('#debug').innerText()))return;}
  throw new Error(`unable to acquire a real block target: ${await page.locator('#debug').innerText()}`);
}
...
test('singleplayer successful mining consumes persisted wooden-pickaxe durability and final use breaks it',async({page})=>{
  ...
  await lockAndAimAtBlock(page);await page.mouse.down({button:'left'});await expect(slot0).toHaveAttribute('data-durability-damage','58',{timeout:5_000});await page.mouse.up({button:'left'});await expect(slot0).toHaveAttribute('data-durability-remaining','1');
  await lockAndAimAtBlock(page);await page.mouse.down({button:'left'});await expect(slot0).not.toHaveAttribute('data-durability-damage',/./,{timeout:5_000});await page.mouse.up({button:'left'});await expect(slot0.locator('.slot-durability')).toHaveCount(0);await expect(slot0.locator('img[alt="木镐"]')).toHaveCount(0);
});
