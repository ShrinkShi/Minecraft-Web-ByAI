...
async function lockAndLookDown(page){
  const canvas=page.locator('#game-canvas');await canvas.click({position:{x:8,y:8}});await expect.poll(()=>page.evaluate(()=>document.pointerLockElement?.id||null),{timeout:5_000}).toBe('game-canvas');await page.evaluate(()=>globalThis.__minecraftE2E?.setLook(0,-1.45));
}
...
test('singleplayer successful mining consumes persisted wooden-pickaxe durability and final use breaks it',async({page})=>{
  ...
  await lockAndLookDown(page);await page.mouse.down({button:'left'});await expect(slot0).toHaveAttribute('data-durability-damage','58',{timeout:5_000});await page.mouse.up({button:'left'});await expect(slot0).toHaveAttribute('data-durability-remaining','1');
  await expect.poll(()=>page.evaluate(()=>document.pointerLockElement?.id||null),{timeout:5_000}).toBe('game-canvas');await page.evaluate(()=>globalThis.__minecraftE2E?.setLook(0,-1.45));await page.mouse.down({button:'left'});await expect(slot0).not.toHaveAttribute('data-durability-damage',/./,{timeout:5_000});await page.mouse.up({button:'left'});await expect(slot0.locator('.slot-durability')).toHaveCount(0);await expect(slot0.locator('img[alt="木镐"]')).toHaveCount(0);
});
