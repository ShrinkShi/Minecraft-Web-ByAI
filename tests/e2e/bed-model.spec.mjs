...
test('bed blocks leave cube mesh, preserve neighbor faces, and build texture-backed halves',async({page})=>{
  ...
  const result=await page.evaluate(async()=>{
    ...
    data[index(5,4,5)]=3; // stone
    data[index(6,4,5)]=BED_IDS.east.foot; // partial visual neighbor
    ...
    for(const [facing,entry] of Object.entries(BED_IDS)){
      for(const part of['foot','head']){
        const descriptor=bedVisualDescriptor(0,0,0,entry[part]),visual=renderer.create(descriptor);let meshes=0,mapped=0,uvMin=Infinity,uvMax=-Infinity,maxY=0;
        visual.traverse(object=>{if(!object.isMesh)return;meshes++;if(object.material?.map)mapped++;const uv=object.geometry.getAttribute('uv'),position=object.geometry.getAttribute('position');for(const value of uv.array){uvMin=Math.min(uvMin,value);uvMax=Math.max(uvMax,value);}for(let i=1;i<position.array.length;i+=3)maxY=Math.max(maxY,position.array[i]);});
        models[`${facing}:${part}`]={meshes,mapped,uvMin,uvMax,maxY,rotationY:visual.rotation.y,texture:visual.children[0]?.material?.map?.image?.src||''};
      }
    }
    renderer.dispose();
    return{
      specials:workerResult.specials,
      opaqueEmpty:workerResult.opaque.empty,
      opaqueIndexBytes:workerResult.opaque.empty?0:workerResult.opaque.indices.byteLength,
      models,
      disposed:{geometries:renderer.geometries.size,templates:renderer.templates.size}
    };
  });

  expect(result.opaqueEmpty).toBeFalsy();
  // One isolated stone emits six faces = 36 uint32 indices. The adjacent bed
  // must not hide the east face as a full cube would.
  ...
  for(const [key,model] of Object.entries(result.models)){
    ...
    expect(model.texture,`${key} must load the tracked Minecraft red-bed sheet`).toContain('/assets/minecraft/textures/entity/bed/red.png');
  }
