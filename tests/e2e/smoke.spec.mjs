...
test('boots, swims, switches precipitation, persists armor, then clears equipment on survival void death',async({page})=>{
  ...
  await expect.poll(async()=>{const record=(await savedWorlds(page)).find(world=>world.name==='CI Browser Smoke');if(!record)return null;return{version:record.version,terrainVersion:record.terrainVersion,chest:record.equipment?.slots?.chest?.id||null,weather:record.weather,hasTransientAir:Object.hasOwn(record,'oxygen')};},{timeout:15_000,message:'pause save should drain to the latest armor/weather snapshot'}).toEqual({version:SINGLEPLAYER_SAVE_VERSION,terrainVersion:4,chest:'leather_chestplate',weather:'clear',hasTransientAir:false});
...
test('custom spawnpoint persists and explicit respawn returns to it',async({page})=>{
  ...
  await expect.poll(async()=>{const record=(await savedWorlds(page)).find(world=>world.name==='CI Custom Respawn');if(!record||Number(record.updatedAt)<saveStarted||!record.respawnPoint)return null;return{version:record.version,terrainVersion:record.terrainVersion,x:Number(record.respawnPoint.x.toFixed(2)),y:Number(record.respawnPoint.y.toFixed(2)),z:Number(record.respawnPoint.z.toFixed(2))};},{timeout:10_000,message:'spawnpoint should persist in the current terrain-v4 world snapshot'}).toEqual({version:SINGLEPLAYER_SAVE_VERSION,terrainVersion:4,...custom});
...
test('bed placement persists respawn, sleeps, and blocks sleep near a real hostile',async({page})=>{
  ...
  await expect.poll(async()=>{const record=(await savedWorlds(page)).find(world=>world.name==='CI Bed Anchor');if(!record||!record.respawnPoint)return null;const ids=Object.values(record.edits||{}).flat().map(entry=>Number(entry?.[1])).filter(id=>id>=11&&id<=18).sort((a,b)=>a-b);return{version:record.version,terrainVersion:record.terrainVersion,validPair:['11,12','13,14','15,16','17,18'].includes(ids.join(',')),hasRespawn:true};},{timeout:15_000,message:'bed pair and respawn anchor should persist before sleep-safety checks'}).toEqual({version:SINGLEPLAYER_SAVE_VERSION,terrainVersion:4,validPair:true,hasRespawn:true});
  ...
  await key(page,'Escape');await expect(page.locator('#pause-menu')).toHaveClass(/active/);
  await expect.poll(async()=>{const record=(await savedWorlds(page)).find(world=>world.name==='CI Bed Anchor');if(!record||!record.respawnPoint)return null;const ids=Object.values(record.edits||{}).flat().map(entry=>Number(entry?.[1])).filter(id=>id>=11&&id<=18).sort((a,b)=>a-b);return{version:record.version,terrainVersion:record.terrainVersion,validPair:['11,12','13,14','15,16','17,18'].includes(ids.join(',')),hasRespawn:true};},{timeout:15_000,message:'valid bed pair and respawn anchor should persist together'}).toEqual({version:SINGLEPLAYER_SAVE_VERSION,terrainVersion:4,validPair:true,hasRespawn:true});
