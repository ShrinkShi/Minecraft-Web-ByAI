...
try{
  ...
  tick();const burning=await waitFor(()=>client.furnaceSnapshot?.cookProgress===1&&client.furnaceSnapshot?.revision===3&&client.furnaceSnapshot?.lit?client.furnaceSnapshot:null,'first furnace burn tick');assert.equal(burning.revision,3);tick();const timerOnly=await waitFor(()=>client.furnaceSnapshot?.cookProgress===2?client.furnaceSnapshot:null,'timer-only furnace progress');assert.equal(timerOnly.revision,3,'timer-only furnace progress must retain the transaction revision');assert.equal(client.state,'ready','same-revision timer snapshots must not trip websocket protocol guards');
