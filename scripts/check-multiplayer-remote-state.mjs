...
const inventory=session=>encodeServerInventorySnapshot({session,revision:0,mode:'survival',slots:Array(36).fill(null)});
...
movement.connect('wss://example.test/ws');socket.open();socket.message(encodeServerWelcome('s:self'));socket.message(info('s:self'));sync(socket,'s:self');socket.message(inventory('s:self'));socket.message(self('s:self'));assert.equal(movement.ready,true);
...
console.log('remote player buffering/renderer lifecycle remains valid behind world-edit + inventory bootstrap barrier: PASS');
