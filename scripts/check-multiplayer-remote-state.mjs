...
import {encodeServerPlayerHungerSnapshot} from '../src/server-player-hunger-snapshot.js';
...
const hunger=session=>encodeServerPlayerHungerSnapshot({session,revision:0,mode:'survival',difficulty:'normal',naturalRegeneration:true,food:20,saturation:5,exhaustion:0,timer:0,statusEffects:[],foodUse:{active:false}});
...
const socket=new FakeSocket(),events=[];const movement=new MultiplayerMovementSession({bootstrapOptions:{clientFactory:options=>new LiveWorldWebSocketClient({...options,socketFactory:()=>socket})},onRemotePlayerSpawn:value=>events.push(['spawn',value.playerId,value.tick]),onRemotePlayerSnapshot:value=>events.push(['snapshot',value.playerId,value.tick]),onRemotePlayerDespawn:value=>events.push(['despawn',value.playerId])});
movement.connect('wss://example.test/ws');socket.open();socket.message(encodeServerWelcome('s:self'));socket.message(info('s:self'));sync(socket,'s:self');socket.message(inventory('s:self'));socket.message(equipment('s:self'));socket.message(crafting('s:self'));socket.message(hunger('s:self'));socket.message(self('s:self'));assert.equal(movement.ready,true);
