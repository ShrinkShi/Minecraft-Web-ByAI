...
movement.connect('wss://example.test/ws');socket.open();socket.message(encodeServerWelcome('s:self'));socket.message(info('s:self'));sync(socket,'s:self');socket.message(inventory('s:self'));socket.message(equipment('s:self'));socket.message(crafting('s:self'));socket.message(self('s:self'));assert.equal(movement.ready,true);
...
console.log('remote player buffering/renderer lifecycle remains valid behind terrain-v3 world-edit + inventory + equipment + player crafting bootstrap barrier: PASS');