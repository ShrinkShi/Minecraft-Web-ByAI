...
try{
  ...
  const initial=decodeServerPlayerSnapshot(await nextKind(messages,SERVER_PLAYER_SNAPSHOT_KIND,'placement initial snapshot'),{expectedSession:welcome.session});assert.equal(initial.mode,'creative');
  const inventory=decodeServerInventorySnapshot(await nextKind(messages,SERVER_INVENTORY_SNAPSHOT_KIND,'placement initial inventory'),{expectedSession:welcome.session});assert.equal(inventory.mode,'creative');assert.equal(inventory.revision,0);
  ...
  const clear=runtime.setBlock(x,y,placeZ,BLOCK.AIR);if(clear.changed){const wire=decodeWorldEditReplication(await nextKind(messages,WORLD_BLOCK_CHANGE_KIND,'placement clear change'),{expectedSession:welcome.session,expectedWorldId:'creative-placement-runtime'});assert.equal(wire.revision,clear.revision);}
  const anchor=runtime.setBlock(x,y,anchorZ,BLOCK.STONE);assert.equal(anchor.changed,true);const anchorWire=decodeWorldEditReplication(await nextKind(messages,WORLD_BLOCK_CHANGE_KIND,'placement anchor change'),{expectedSession:welcome.session,expectedWorldId:'creative-placement-runtime'});assert.equal(anchorWire.kind,WORLD_BLOCK_CHANGE_KIND);assert.equal(anchorWire.revision,anchor.revision);assert.equal(runtime.world.getBlock(x,y,placeZ),BLOCK.AIR);
  ...
  tick();const snapshot=decodeServerPlayerSnapshot(await nextKind(messages,SERVER_PLAYER_SNAPSHOT_KIND,'placement tick snapshot'),{expectedSession:welcome.session});assert.equal(snapshot.tick,1);assert.ok(Math.abs(snapshot.yaw-Math.PI/2)<1e-12,'player simulation should use the newer live view');
  const placed=decodeWorldEditReplication(await nextKind(messages,WORLD_BLOCK_CHANGE_KIND,'placement authoritative block change'),{expectedSession:welcome.session,expectedWorldId:'creative-placement-runtime'});assert.equal(placed.kind,WORLD_BLOCK_CHANGE_KIND);assert.deepEqual({x:placed.x,y:placed.y,z:placed.z,previous:placed.previous,id:placed.id},{x,y,z:placeZ,previous:BLOCK.AIR,id:BLOCK.GRASS},'queued use must place the block selected and aimed at when use was accepted, not the later hotbar/view state');assert.equal(placed.revision,(revisionBeforeUse+1)>>>0);assert.equal(runtime.world.getBlock(x,y,placeZ),BLOCK.GRASS);assert.deepEqual(errors,[]);
