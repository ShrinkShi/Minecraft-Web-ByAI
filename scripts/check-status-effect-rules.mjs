...
const close=(actual,expected,epsilon=1e-9,message=`${actual} != ${expected}`)=>assert.ok(Math.abs(actual-expected)<=epsilon,message);
...
stepped=stepStatusEffects([{id:'hunger',amplifier:0,remainingSeconds:2}],{dt:5});assert.deepEqual(stepped.effects,[]);close(stepped.hungerExhaustion,.2,1e-9,'expired effects must contribute only for their remaining active duration');
