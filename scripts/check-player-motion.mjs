...
const dry=planPlayerMotionStep({...base,dt:.1});assert.equal(dry.dt,.05,'long frames are clamped to the existing 50ms physics step');close(dry.displacement.x,0);close(dry.displacement.z,-PLAYER_WALK_SPEED*.05);close(dry.velocity.y,-PLAYER_GRAVITY*.05);close(dry.displacement.y,-PLAYER_GRAVITY*.05*.05);close(dry.horizontalDrag,Math.exp(-PLAYER_GROUND_HORIZONTAL_DRAG*.05));assert.equal(dry.swimActive,false);
...
const sprint=planPlayerMotionStep({...base,dt:.05,control:{...base.control,sprint:true}});close(Math.hypot(sprint.displacement.x,sprint.displacement.z),PLAYER_SPRINT_SPEED*.05);
...
const flight=planPlayerMotionStep({...base,dt:.05,flying:true,swimCoverage:0,control:{...base.control,jump:true,sprint:true}});assert.equal(flight.swimActive,false);close(flight.displacement.z,-PLAYER_SPRINT_SPEED*.05);close(flight.displacement.y,PLAYER_FLIGHT_VERTICAL_SPEED*.05);assert.deepEqual(flight.velocity,{x:0,y:0,z:0});assert.equal(flight.horizontalDrag,1);
...
const swim=planPlayerMotionStep({...base,dt:.05,swimCoverage:1});assert.equal(swim.swimActive,true);close(Math.hypot(swim.displacement.x,swim.displacement.z),PLAYER_WALK_SPEED*.5*.05);close(swim.horizontalDrag,Math.exp(-PLAYER_SWIM_HORIZONTAL_DRAG*.05));assert.ok(swim.velocity.y>0,'full water coverage applies buoyancy');
