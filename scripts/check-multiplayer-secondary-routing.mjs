...
const calls=[],drops=[];let fallback=0;
...
assert.throws(()=>registerControlActionInterceptor(null),/interceptor/);assert.throws(()=>installMultiplayerSecondaryRouting({runtime:{player:{mode:'creative',yaw:0,pitch:0}},movement:{}}),/sendUse/);assert.throws(()=>installMultiplayerSecondaryRouting({runtime:{player:{mode:'creative',yaw:0,pitch:0}},movement:{sendUse(){}}}),/sendDrop/);

console.log('scoped multiplayer secondary + drop routing lifecycle: PASS');
