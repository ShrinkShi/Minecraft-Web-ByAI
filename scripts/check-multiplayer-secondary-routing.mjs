...
let secondaryFirst=0,secondarySecond=0;const releaseSecondaryFirst=registerControlSecondaryInterceptor(()=>{secondaryFirst++;return undefined;}),releaseSecondarySecond=registerControlSecondaryInterceptor(()=>{secondarySecond++;return false;});bus.setSecondary('mouse',true);assert.equal(secondaryFirst,1);assert.equal(secondarySecond,1);assert.equal(secondaryFallback,2,'a later secondary interceptor may explicitly handle the transition');releaseSecondaryFirst();releaseSecondarySecond();bus.setSecondary('mouse',false);assert.equal(secondaryFallback,3);
