...
movement.ready=true;runtime.player.yaw=-1.5;runtime.player.pitch=.4;assert.equal(bus.action('touch','secondary'),true);assert.deepEqual(calls[2],{yaw:-1.5,pitch:.4},'routing must snapshot the current local view when the action is emitted');
assert.equal(release(),true);assert.equal(release(),false,'routing cleanup must be idempotent');bus.setButton('mouse','primary',true);bus.setButton('mouse','primary',false);assert.equal(primaryFallback,2,'disposing multiplayer routing must restore the original primary callback');assert.equal(bus.action('mouse','secondary'),'fallback:secondary');assert.equal(fallback,2,'disposing multiplayer routing must restore the original action callback');assert.equal(bus.action('keyboard','drop'),'fallback:drop');assert.equal(fallback,3,'disposing multiplayer routing must restore legacy drop handling too');
...
console.log('scoped multiplayer creative/survival secondary + drop + primary routing lifecycle: PASS');
