...
const partial=items.spawn('stick',2,{x:0,y:2,z:0},{velocity:{x:0,y:0,z:0},pickupDelay:0}),beforePartialEvents=events.length;const partialResult=items.step(world,[{session:'s:pickup',mode:'survival',position:{x:0,y:1,z:0}}],{dt:.05,onPickup:()=>1});assert.equal(partialResult.pickups,1);assert.equal(partialResult.snapshots,1,'a partial pickup must emit exactly one authoritative snapshot in the tick');assert.equal(events.length,beforePartialEvents+1);assert.equal(events.at(-1)[0],'snapshot');assert.equal(events.at(-1)[1].revision,1);assert.equal(events.at(-1)[1].count,1);items.remove(partial.entityId,'removed');
const spawned=items.spawn('stick',2,{x:0,y:2,z:0},{velocity:{x:0,y:0,z:0},pickupDelay:0});assert.equal(spawned.revision,0);assert.equal(items.size,1);
