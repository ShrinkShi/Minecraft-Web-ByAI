...
try{
  ...
  const initial=decodeServerPlayerSnapshot(await nextKind(messages,SERVER_PLAYER_SNAPSHOT_KIND,'creative initial snapshot'),{expectedSession:welcome.session});assert.equal(initial.mode,'creative');
  const inventory=decodeServerInventorySnapshot(await nextKind(messages,SERVER_INVENTORY_SNAPSHOT_KIND,'creative initial inventory'),{expectedSession:welcome.session});assert.equal(inventory.mode,'creative');assert.equal(inventory.revision,0);
  ...
  for(const change of [runtime.setBlock(x,y,frontZ,BLOCK.AIR),runtime.setBlock(x,y,targetZ,BLOCK.STONE)]){
    if(!change.changed)continue;const wire=decodeWorldEditReplication(await nextKind(messages,WORLD_BLOCK_CHANGE_KIND,'creative setup block change'),{expectedSession:welcome.session,expectedWorldId:'creative-break-runtime'});assert.equal(wire.kind,WORLD_BLOCK_CHANGE_KIND);assert.equal(wire.revision,change.revision);
  }
