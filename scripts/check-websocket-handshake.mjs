...
assert.equal(MULTIPLAYER_HANDSHAKE_VERSION,5);assert.equal(MULTIPLAYER_SUBPROTOCOL,'minecraft-web-v5');assert.ok(SERVER_REJECT_CODES.includes('protocol-mismatch'));
...
assert.equal(isCompatibleServerHandshake({v:MULTIPLAYER_HANDSHAKE_VERSION,kind:'welcome',session:'ok'}),true);assert.equal(isCompatibleServerHandshake({v:4,kind:'welcome',session:'legacy-v4'}),false);
assert.throws(()=>decodeClientHello({v:4,kind:'hello'}),/unsupported multiplayer handshake version/);assert.throws(()=>decodeServerHandshake({v:4,kind:'welcome',session:'legacy-v4'}),/unsupported multiplayer handshake version/);
...
console.log('strict websocket v5 hello/welcome + authoritative flight snapshot transport contracts: PASS');
