...
assert.equal(MULTIPLAYER_HANDSHAKE_VERSION,6);assert.equal(MULTIPLAYER_SUBPROTOCOL,'minecraft-web-v6');assert.ok(SERVER_REJECT_CODES.includes('protocol-mismatch'));
...
assert.equal(isCompatibleServerHandshake({v:MULTIPLAYER_HANDSHAKE_VERSION,kind:'welcome',session:'ok'}),true);assert.equal(isCompatibleServerHandshake({v:5,kind:'welcome',session:'legacy-v5'}),false);assert.equal(isCompatibleServerHandshake({v:4,kind:'welcome',session:'legacy-v4'}),false);
assert.throws(()=>decodeClientHello({v:5,kind:'hello'}),/unsupported multiplayer handshake version/);assert.throws(()=>decodeServerHandshake({v:5,kind:'welcome',session:'legacy-v5'}),/unsupported multiplayer handshake version/);
...
const socket=created[0];socket.open();assert.equal(client.state,'handshaking');assert.deepEqual(JSON.parse(socket.sent[0]),encodeClientHello());assert.throws(()=>client.sendInput('view',{}),/not ready/);
...
console.log('strict websocket v6 hello/welcome + authoritative flight snapshot transport contracts: PASS');