...
client.connect('wss://localhost/ws');socket.open();socket.message(encodeServerWelcome('s:live'));socket.message(encodeServerWorldInfo({session:'s:live',worldId:'live-world',terrainVersion:1,seed:'seed',prompt:'plain',tickRate:20}));for(const message of encodeWorldEditSync({session:'s:live',worldId:'live-world',revision:7,edits:[]}))socket.message(message);assert.equal(client.worldRevision,7);socket.message(encodeWorldBlockChange({session:'s:live',worldId:'live-world',revision:8,x:1,y:20,z:1,previous:BLOCK.AIR,id:BLOCK.STONE}));assert.equal(changes.length,1);assert.equal(changes[0].revision,8);assert.equal(client.worldRevision,8);
console.log('live world client wiring: PASS');
