...
const bulk=new ServerPlayerInventoryState('s:inventory-bulk',{mode:'survival'});assert.equal(bulk.add('stick',130),0);assert.equal(bulk.snapshot().revision,1);assert.deepEqual(bulk.snapshot().slots.slice(0,3),[{id:'stick',count:64},{id:'stick',count:64},{id:'stick',count:2}]);
...
const creative=new ServerPlayerInventoryState('s:creative-catalog',{mode:'creative'});const creativeBefore=creative.snapshot().revision;result=creative.creativePick('wooden_pickaxe');assert.equal(result.changed,true);assert.equal(result.reason,'creative-picked');assert.equal(result.snapshot.revision,creativeBefore+1);assert.deepEqual(result.snapshot.cursor,{id:'wooden_pickaxe',count:1},'server chooses the registered max stack');const unchangedSlots=result.snapshot.slots.map(stack=>stack?{...stack}:null);result=creative.creativePick('block:1');assert.equal(result.changed,true);assert.deepEqual(result.snapshot.cursor,{id:'block:1',count:64});assert.deepEqual(result.snapshot.slots,unchangedSlots,'creative catalog pick must not rewrite physical slots');
const creativeRevision=result.snapshot.revision;result=creative.creativePick('missing:item');assert.equal(result.changed,false);assert.equal(result.reason,'unknown-item');assert.equal(result.snapshot.revision,creativeRevision);assert.deepEqual(result.snapshot.cursor,{id:'block:1',count:64});creative.setMode('survival');const survivalRevision=creative.snapshot().revision;result=creative.creativePick('stick');assert.equal(result.changed,false);assert.equal(result.reason,'creative-only');assert.equal(result.snapshot.revision,survivalRevision);assert.deepEqual(result.snapshot.cursor,{id:'block:1',count:64},'rejected non-creative pick does not mutate cursor');

console.log('authoritative inventory bulk insert + cursor/click/shift/return + creative pick state machine: PASS');
