...
const wire=(session='s:inventory-client')=>encodeServerInventorySnapshot({session,revision:3,mode:'survival',slots:Array(36).fill(null)});
...
socket.message(wire());assert.equal(client.state,'error');assert.equal(socket.closed.at(-1).code,1002);assert.match(errors.at(-1),/duplicate initial inventory snapshot/);assert.equal(client.inventorySnapshot,null,'protocol failure clears retained inventory state');
...
console.log('websocket strict initial inventory snapshot handling: PASS');
