...
import {encodeInventoryTransactionResult,INVENTORY_TRANSACTION_REQUEST_KIND} from '../src/inventory-transaction-wire.js';
...
const second=client.sendInventoryTransaction({type:'return-cursor'},7);assert.equal(second.requestId,1);assert.equal(second.expectedRevision,7);
socket.message(encodeInventoryTransactionResult({session:'s:inventory-client',requestId:1,ok:false,code:'stale-revision',revision:8}));assert.equal(results.length,2);assert.equal(client.state,'ready');
socket.message(encodeInventoryTransactionResult({session:'s:inventory-client',requestId:1,ok:true,code:'no-change',revision:8}));assert.equal(client.state,'error');assert.equal(socket.closed.at(-1).code,1002);assert.match(errors.at(-1),/unexpected inventory transaction result requestId/);

console.log('websocket inventory transaction serialization + result correlation: PASS');
