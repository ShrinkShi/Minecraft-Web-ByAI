...
function testMobRules(){
  ...
  assert.deepEqual(Object.keys(HOSTILE_MOBS),['zombie','skeleton']);assert.equal(chooseHostileMob(()=>0),'zombie');assert.equal(chooseHostileMob(()=>.99),'skeleton');assert.equal(HOSTILE_MOBS.zombie.attackStyle,'melee');assert.equal(HOSTILE_MOBS.skeleton.attackStyle,'ranged');assert.ok(HOSTILE_MOBS.skeleton.projectileSpeed>0);
