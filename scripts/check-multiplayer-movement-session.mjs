...
bootstrapHooks.onReady({client:bootstrap.client,worldInfo:{session:'s:move',worldId:'movement-world',tickRate:20},worldEdits:{session:'s:move',worldId:'movement-world',revision:5,edits:{'17,20,-1':3}},initialSnapshot:initial});
assert.equal(movement.ready,true);assert.equal(ready.length,1);assert.equal(ready[0].worldEdits.revision,5);assert.equal(ready[0].worldEdits.edits['17,20,-1'],3);assert.equal(ready[0].initialSnapshot.tick,0);assert.deepEqual(movement.current().position,{x:.5,y:25.001,z:.5});assert.equal(bridgeCalls[0][0],'prime');assert.equal(bridgeCalls[1][0],'flush');
