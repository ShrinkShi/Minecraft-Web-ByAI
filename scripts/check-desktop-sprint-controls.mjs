...
controls.onKeyDown(event('KeyW',1000));const leftCtrl=event('ControlLeft',1010);controls.onKeyDown(leftCtrl);assert.equal(leftCtrl.defaultPrevented,true,'gameplay Ctrl sprint must suppress the browser chord');assert.equal(sourceState.sprint,true,'Left Ctrl + W should sprint');controls.onKeyUp(event('ControlLeft',1020));assert.equal(sourceState.sprint,false);controls.onKeyUp(event('KeyW',1030));
