...
const absentTransport=new AuthoritativeWorldSession({world:new ServerTerrainWorld({seed:'golden-seed',prompt:'mountain forest'}),getInputState:session=>inputState(session),sendPlayerSnapshot:()=>null});absentTransport.join('offline');const offlineTick=absentTransport.tickOnce()[0];assert.equal(offlineTick.stepped,true);assert.equal(offlineTick.sent,false);assert.equal(offlineTick.reason,'transport-unavailable');assert.equal(absentTransport.hasSession('offline'),true,'a transient null send does not guess that the session is closed');
