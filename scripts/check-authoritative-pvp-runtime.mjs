...
async function ready(client,url,label){client.connect(url);await waitFor(()=>client.session&&client.worldInfo&&client.worldEditSync&&client.inventorySnapshot&&client.playerCraftingSnapshot&&client.combatSnapshot,`${label} bootstrap`);return client.session;}
...
console.log('real two-client PvP damage + knockback + death drops + dead pickup exclusion + respawn: PASS');
