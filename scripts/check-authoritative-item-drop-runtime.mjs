...
try{
  ...
  const initial=decodeServerPlayerSnapshot(await messages.next('item drop initial player'),{expectedSession:welcome.session});assert.equal(initial.mode,'survival');const initialInventory=decodeServerInventorySnapshot(await messages.next('item drop initial inventory'),{expectedSession:welcome.session});assert.equal(initialInventory.revision,0);assert.equal(initialInventory.slots[HOTBAR_START],null);
  ...
  tick();const playerWire=decodeServerPlayerSnapshot(await messages.next('item drop tick player'),{expectedSession:welcome.session});assert.equal(playerWire.tick,1);
  const inventoryWire=decodeServerInventorySnapshot(await messages.next('item drop inventory mutation'),{expectedSession:welcome.session});assert.equal(inventoryWire.revision,2);assert.equal(inventoryWire.slots[HOTBAR_START],null,'survival drop must consume the server-owned selected hotbar item');assert.equal(runtime.inventories.snapshot(welcome.session).slots[HOTBAR_START],null);
  const spawn=decodeItemEntityReplication(await messages.next('item drop entity spawn'));assert.equal(spawn.kind,ITEM_ENTITY_SPAWN_KIND);assert.equal(spawn.entityId,'i:runtime_drop');assert.equal(spawn.itemId,'stick');assert.equal(spawn.count,1);assert.equal(runtime.itemEntities.size,1);
  const moved=decodeItemEntityReplication(await messages.next('item drop first entity snapshot'));assert.equal(moved.kind,ITEM_ENTITY_SNAPSHOT_KIND);assert.equal(moved.entityId,spawn.entityId);assert.equal(moved.revision,1);assert.deepEqual(errors,[]);
