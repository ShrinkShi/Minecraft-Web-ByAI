...
async function nextKind(messages,kind,label){for(let i=0;i<32;i++){const message=await messages.next(label);if(message.kind===kind)return message;}throw new Error(`did not receive ${kind}`);}
...
try{
  ...
  const initialInventory=decodeServerInventorySnapshot(await nextKind(messages,SERVER_INVENTORY_SNAPSHOT_KIND,'initial inventory snapshot'),{expectedSession:welcome.session});assert.equal(initialInventory.mode,'creative');assert.equal(initialInventory.revision,0);assert.equal(initialInventory.slots.length,INVENTORY_SLOT_COUNT);assert.equal(initialInventory.slots[HOTBAR_START].id,CREATIVE_START[0]);
