...
try{
  ...
  const bridge=new MultiplayerInputBridge({transport:client,isReady:()=>client.state==='ready',viewProvider:()=>({yaw:0,pitch:0})});bridge.sendUse({yaw:0,pitch:0});const opened=await waitForTick(()=>client.workbenchSnapshot,'workbench open');assert.equal(opened.containerId,'w:runtime');assert.deepEqual(opened.target,table);assert.equal(opened.revision,0);assert.equal(runtime.workbenches.hub.sessionCount,1);
  ...
  bridge.sendUse({yaw:0,pitch:0});await waitForTick(()=>client.workbenchSnapshot?.containerId==='w:runtime','workbench reopen');runtime.setBlock(table.x,table.y,table.z,BLOCK.AIR);await waitForTick(()=>client.workbenchSnapshot===null,'forced close after table removal');assert.equal(runtime.workbenches.hub.sessionCount,0);assert.equal(errors.length,0);
