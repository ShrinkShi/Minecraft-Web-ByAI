...
const beforeFlightToggle=transport.sent.length,flightToggle=bridge.sendFlightToggle();assert.equal(transport.sent.length,beforeFlightToggle+1,'flight toggle must emit exactly one payloadless action and no view packet');assert.equal(transport.sent.at(-1).kind,'action');assert.equal(flightToggle.kind,'flight-toggle');assert.equal(flightToggle.seq,0);assert.equal('viewSeq' in flightToggle,false);assert.equal(bridge.actionSeq,1);
assert.throws(()=>new MultiplayerInputBridge({transport:null,viewProvider:()=>currentView}),/transport/);assert.throws(()=>new MultiplayerInputBridge({transport:{},viewProvider:()=>currentView}),/sendInput/);assert.throws(()=>new MultiplayerInputBridge({transport,viewProvider:null}),/viewProvider/);assert.throws(()=>bridge.setView({yaw:0,pitch:2}),/pitch is out of range/);assert.throws(()=>bridge.sendReferencedAction('inventory'),/use, drop, or attack/);assert.throws(()=>bridge.sendPayloadlessAction('drop'),/use-release, respawn, or flight-toggle/);transport.state='ready';assert.throws(()=>bridge.sendHotbarSelect(9),/0 to 8/);

console.log('coalesced runtime multiplayer control/view/action bridge + payloadless creative flight toggle: PASS');
