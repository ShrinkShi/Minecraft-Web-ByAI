...
assert.deepEqual(zeroOffsets[0],{dx:-1,dz:-1});
...
assert.deepEqual(boneMealGrassCandidateOffsets(()=>.999999,{attempts:1,maxRadius:4}),[{dx:1,dz:1}]);
assert.deepEqual(boneMealGrassCandidateOffsets(()=>.5,{attempts:0,maxRadius:4}),[]);
assert.throws(()=>rollShortGrassDrops(()=>1),/\[0, 1\)/);
assert.throws(()=>boneMealGrassCandidateOffsets(()=>NaN,{attempts:1}),/\[0, 1\)/);
