...
let legacyDrops=0,legacyWear=0;const legacy=new SingleplayerMiningController({aim:()=>({x:4,y:20,z:4,id:BLOCK.DIRT,previous:{x:4,y:20,z:5}}),getMode:()=> 'survival',getSelectedStack:()=>({id:'removed_mod_item',count:1}),breakTarget:()=>true,spawnDrop:stack=>{legacyDrops++;assert.deepEqual(stack,{id:'block:2',count:1});},damageSelected:()=>legacyWear++});legacy.start(0);legacy.step(0);for(let t=50;t<=450;t+=50)legacy.step(t);assert.equal(legacyDrops,1,'legacy unknown held items should behave like generic hands for harvestable blocks');assert.equal(legacyWear,0,'legacy unknown held items must never enter strict durability mutation');
...
console.log('singleplayer mining parity + legacy-held compatibility + durability ordering: PASS');
