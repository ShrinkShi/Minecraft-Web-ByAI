...
assert.equal(PLAYER_COLLISION_RADIUS,.3);assert.equal(PLAYER_COLLISION_HEIGHT,1.8);assert.equal(PLAYER_EYE_HEIGHT,1.62);assert.equal(PLAYER_COLLISION_EPSILON,.001);assert.equal(PLAYER_GROUND_PROBE_DISTANCE,.06);assert.equal(PLAYER_WATER_EXIT_STEP_HEIGHT,.6);assert.deepEqual(PLAYER_WATER_SAMPLE_OFFSETS,[.2,.9,1.62]);assert.deepEqual(PLAYER_MOVE_AXES,['x','y','z']);
...
const steppedX=resolvePlayerAxisMove({...initial,axis:'x',amount:.4,stepHeight:PLAYER_WATER_EXIT_STEP_HEIGHT,collides:position=>position.x>.7&&position.y<1.5});
assert.deepEqual(steppedX,{position:{x:.9,y:1.6,z:.5},velocity:{x:2,y:0,z:4},grounded:false,moved:true,blocked:false,stepped:true},'jumping swimmer can clear a low shoreline ledge without remaining pinned against it');
const blockedStep=resolvePlayerAxisMove({...initial,axis:'x',amount:.4,stepHeight:PLAYER_WATER_EXIT_STEP_HEIGHT,collides:position=>position.x>.7||position.y>1.5});assert.equal(blockedStep.blocked,true,'water step fails closed when the raised AABB or destination is obstructed');
...
assert.throws(()=>playerBlockBounds({x:0,y:0,z:0},{radius:0}),/radius must be > 0/);assert.throws(()=>playerBlockBounds({x:0,y:0,z:0},{epsilon:.3}),/epsilon must be smaller/);assert.throws(()=>playerCollidesBlocks({x:0,y:0,z:0},null),/isSolidBlock must be a function/);assert.throws(()=>resolvePlayerAxisMove({...initial,axis:'q',amount:1,collides:()=>false}),/axis must be x, y or z/);assert.throws(()=>resolvePlayerAxisMove({...initial,axis:'x',amount:NaN,collides:()=>false}),/amount must be a finite number/);assert.throws(()=>resolvePlayerAxisMove({...initial,axis:'x',amount:1,stepHeight:-1,collides:()=>false}),/stepHeight must be >= 0/);assert.throws(()=>probePlayerGrounded({x:0,y:0,z:0},()=>false,0),/distance must be > 0/);assert.throws(()=>samplePlayerWaterCoverage({x:0,y:0,z:0},()=>false,{eyeHeight:0}),/eyeHeight must be > 0/);

console.log('shared player AABB collision + grounded + water-edge exit rules: PASS');
