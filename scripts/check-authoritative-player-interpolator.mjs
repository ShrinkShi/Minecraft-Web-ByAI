...
const remoteState=(tick,overrides={})=>{const value=state(tick,overrides);delete value.flying;return value;};
...
const remoteCompatible=new AuthoritativePlayerInterpolator();remoteCompatible.accept(remoteState(200));assert.equal(Object.prototype.hasOwnProperty.call(remoteCompatible.current(),'flying'),false,'remote snapshot v1 state must not require or invent self-only flight metadata');remoteCompatible.accept(remoteState(201,{position:{x:.1,y:25,z:0},mode:'creative'}));const remoteSample=remoteCompatible.step(.025);assert.equal(remoteSample.mode,'creative');assert.equal(Object.prototype.hasOwnProperty.call(remoteSample,'flying'),false,'remote interpolation must preserve the absence of self-only flight metadata');
...
console.log('authoritative player snapshot interpolation + optional flight metadata + snap/wrap rules: PASS');
