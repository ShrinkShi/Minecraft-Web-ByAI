...
const state=(tick,overrides={})=>({session:'interp-session',tick,position:{x:0,y:25,z:0},velocity:{x:0,y:0,z:0},yaw:0,pitch:0,mode:'survival',grounded:true,swimCoverage:0,voided:false,...overrides});
...
let result=interp.accept(state(0));assert.deepEqual(result,{accepted:true,snapped:true,reason:'initial',tick:0});assert.equal(interp.ready,true);assert.deepEqual(interp.current().position,{x:0,y:25,z:0});
...
const metadata=new AuthoritativePlayerInterpolator();metadata.accept(state(100));metadata.accept(state(101,{position:{x:.1,y:25,z:0},mode:'creative',grounded:false,swimCoverage:.5,voided:true,pitch:.4}));sampled=metadata.step(.01);assert.equal(sampled.mode,'creative');assert.equal(sampled.grounded,false);assert.equal(sampled.swimCoverage,.5);assert.equal(sampled.voided,true);near(sampled.pitch,.08,1e-12,'pitch interpolation');
...
console.log('authoritative player snapshot interpolation + snap/wrap rules: PASS');
