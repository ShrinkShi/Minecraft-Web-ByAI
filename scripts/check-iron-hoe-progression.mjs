...
const item=ITEMS.iron_hoe;assert.equal(item.name,'铁锄');assert.equal(item.stack,1);assert.equal(item.assetKey,'item.iron_hoe');assert.equal(item.attackDamage,1);assert.deepEqual(item.combat,{attackSpeed:3,durabilityCost:2});assert.deepEqual(item.tool,{kind:'hoe',tier:'iron',speed:6,durability:250});assert.equal(itemDurability('iron_hoe'),250);assert.deepEqual(meleeProfile('iron_hoe'),{itemId:'iron_hoe',damage:1,attackSpeed:3,attackIntervalMs:1000/3,durabilityCost:2});
...
console.log('source-backed iron hoe recipe + combat profile + 250 durability + canonical texture contract: PASS');
