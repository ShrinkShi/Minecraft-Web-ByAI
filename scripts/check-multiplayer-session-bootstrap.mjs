...
edits(socket,'s:a',4);assert.equal(bootstrap.state,'ready');assert.equal(ready.length,1);assert.equal(ready[0].worldEdits.revision,4);assert.deepEqual(ready[0].worldEdits.edits['1,2,3'],{id:3,stateKey:null});assert.equal(ready[0].initialSnapshot.tick,0);assert.equal(ready[0].initialSnapshot.flying,false);assert.equal(ready[0].inventorySnapshot.revision,7);assert.equal(ready[0].inventorySnapshot.slots.length,36);assert.equal(ready[0].equipmentSnapshot.revision,3);assert.deepEqual(ready[0].equipmentSnapshot.slots,{head:null,chest:null,legs:null,feet:null});assert.equal(ready[0].playerCraftingSnapshot.revision,2);assert.deepEqual(ready[0].playerCraftingSnapshot.slots,[null,null,null,null]);assert.equal(errors.length,0);
...
console.log('multiplayer bootstrap requires terrain-v3 world-info + block-state-aware initial edits + inventory + equipment + player crafting + authoritative player snapshot v2: PASS');