...
const player=(session,tick=0)=>encodeServerPlayerSnapshot({session,tick,position:{x:.5,y:25.001,z:.5},velocity:{x:0,y:0,z:0},yaw:0,pitch:0,mode:'survival',grounded:true,swimCoverage:0,voided:false});
...
bootstrap.connect('wss://example.test/ws');socket.open();socket.message(encodeServerWelcome('s:a'));socket.message(info('s:a'));socket.message(player('s:a'));
assert.equal(bootstrap.state,'synchronizing');assert.equal(ready.length,0,'world-info + tick0 must not bypass the edit snapshot barrier');
edits(socket,'s:a',4);assert.equal(bootstrap.state,'ready');assert.equal(ready.length,1);assert.equal(ready[0].worldEdits.revision,4);assert.equal(ready[0].worldEdits.edits['1,2,3'],3);assert.equal(ready[0].initialSnapshot.tick,0);assert.equal(errors.length,0);
socket.message(player('s:a',1));assert.equal(ready.length,1);assert.equal(bootstrap.latestSnapshot.tick,1);

const reverseSocket=new FakeSocket(),reverseReady=[];const reverse=new MultiplayerSessionBootstrap({clientFactory:factory(reverseSocket),onReady:value=>reverseReady.push(value)});reverse.connect('wss://example.test/ws');reverseSocket.open();reverseSocket.message(encodeServerWelcome('s:reverse'));reverseSocket.message(player('s:reverse',3));reverseSocket.message(info('s:reverse'));assert.equal(reverse.state,'synchronizing');edits(reverseSocket,'s:reverse');assert.equal(reverse.state,'ready');assert.equal(reverseReady[0].initialSnapshot.tick,3);
...
console.log('multiplayer bootstrap requires world-info + initial edits + authoritative player snapshot: PASS');
