...
const STATUS_EFFECT_SAVE_MIN_VERSION=10;
assert.ok(SINGLEPLAYER_SAVE_VERSION>=STATUS_EFFECT_SAVE_MIN_VERSION,'current save schema must retain v10 status-effect persistence support');
...
assert.equal(resolveSingleplayerTerrainVersion({version:10,terrainVersion:4}),4,'v10 worlds retain the terrain v4 contract that introduced persisted status effects');
assert.equal(resolveSingleplayerTerrainVersion({version:SINGLEPLAYER_SAVE_VERSION,terrainVersion:4}),4,'current saves keep the same terrain v4 contract');
...
assert.throws(()=>resolveSingleplayerTerrainVersion({version:SINGLEPLAYER_SAVE_VERSION}),/missing terrainVersion/);

console.log(`status-effect save floor v${STATUS_EFFECT_SAVE_MIN_VERSION} preserved through current save v${SINGLEPLAYER_SAVE_VERSION} / terrain v4: PASS`);
