...
try{
  ...
  const spawn=decodeItemEntityReplication(await messages.next('mined cobblestone spawn'));assert.equal(spawn.kind,ITEM_ENTITY_SPAWN_KIND);assert.equal(spawn.entityId,'i:mining_drop');assert.equal(spawn.itemId,'block:10');assert.equal(spawn.count,1);const firstDropTick=decodeItemEntityReplication(await messages.next('mined cobblestone first physics snapshot'));assert.equal(firstDropTick.kind,ITEM_ENTITY_SNAPSHOT_KIND);assert.equal(firstDropTick.entityId,spawn.entityId);assert.ok(firstDropTick.pickupDelay>0,'mined block drops must preserve the server pickup delay instead of teleporting directly into Inventory');
