...
let info=inspectBlockTarget(BLOCK.STONE,{selectedItemId:null});assert.equal(info.name,'石头');assert.equal(info.requiredToolName,'镐');assert.equal(info.requiredToolTier,'wood');assert.equal(info.requiredToolTierName,'木质');assert.equal(info.heldToolTier,null);assert.equal(info.toolCorrect,false);assert.equal(info.canDrop,false);assert.equal(info.dropName,'圆石');
...
info=inspectBlockTarget(BLOCK.DIRT,{selectedItemId:null});assert.equal(info.requiredToolName,'任意');assert.equal(info.requiredToolTier,null);assert.equal(info.requiredToolTierName,null);assert.equal(info.toolCorrect,true);assert.equal(info.canDrop,true);assert.equal(info.dropName,'泥土');
...
info=chooseLookTargetInfo({blockHit:{id:BLOCK.STONE,distance:2},entityHit:{entity:cow,distance:3},selectedItemId:'wooden_pickaxe'});assert.equal(info.kind,'block');assert.equal(info.canDrop,true);assert.equal(info.requiredToolTierName,'木质');
console.log('Jade-style block/entity target inspection + harvest tier metadata: PASS');
