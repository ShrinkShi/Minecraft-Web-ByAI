...
import {createBlockStateSidecar} from '../src/block-state-sidecar.js';
...
const stateData=new Uint8Array(CHUNK_SIZE*CHUNK_SIZE*64),stateCalls=[],sidecar=createBlockStateSidecar(),stateWorld={chunks:new Map([['0,0',stateData]]),edits:new Map(),blockStateSidecar:sidecar,index,requestMesh:(x,z)=>stateCalls.push(`${x},${z}`)},stateIndex=index(2,20,3);stateData[stateIndex]=BLOCK.LOG;
...
assert.equal(stateChange.applied,true);assert.equal(stateChange.stateChanged,true);assert.equal(stateData[stateIndex],BLOCK.LOG,'state-only replication must not require a numeric block-id mutation');assert.deepEqual(sidecar.get('0,0',stateIndex,BLOCK.LOG),{id:BLOCK.LOG,stateKey:'axis=x'});assert.equal(stateWorld.edits.get('0,0').get(stateIndex),BLOCK.LOG);assert.deepEqual(stateCalls,['0,0'],'state-only visual changes must rebuild the loaded chunk mesh');
...
assert.throws(()=>applyVoxelOverlay(a,{x:4,y:20,z:4,previous:BLOCK.LOG,previousStateKey:'axis=y',id:BLOCK.LOG,stateKey:'axis=x'}),/blockStateSidecar/);
