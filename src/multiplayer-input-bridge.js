...
export class MultiplayerInputBridge{
  constructor({transport,viewProvider,isReady=null}={}){
    this.transport=object(transport,'transport');if(typeof this.transport.sendInput!=='function')throw new TypeError('transport.sendInput must be a function');
    this.viewProvider=callback(viewProvider,'viewProvider');this.isReady=isReady===null?()=>this.transport.state==='ready':callback(isReady,'isReady');this.reset();
  }
  ...
  flushView(){
    if(!this.isReady())return null;const view=this.pendingView||viewState(this.viewProvider());if(sameView(this.sentView,view)&&this.lastSentViewSequence!==null)return{sequence:this.lastSentViewSequence,reused:true,frame:null};
    const frame=encodePlayerViewFrame(view,this.viewSeq);this.transport.sendInput('view',frame);this.lastSentViewSequence=this.viewSeq;this.viewSeq=nextNetworkSequence(this.viewSeq);this.sentView={yaw:frame.yaw,pitch:frame.pitch};this.pendingView={...this.sentView};return{sequence:frame.seq,reused:false,frame};
  }

  flushControl(){
    if(!this.isReady()||!this.pendingControl||sameControl(this.sentControl,this.pendingControl))return null;
    const frame=encodePlayerControlFrame(this.pendingControl,this.controlSeq);this.transport.sendInput('control',frame);this.controlSeq=nextNetworkSequence(this.controlSeq);this.sentControl=normalizeControlState(this.pendingControl);return frame;
  }
  ...
  sendReferencedAction(kind,view=this.viewProvider()){
    if(kind!=='use'&&kind!=='drop')throw new RangeError('referenced multiplayer action must be use or drop');
    if(!this.isReady())return null;this.setView(view);const viewResult=this.flushView();if(!viewResult)throw new Error('unable to establish authoritative action view');
    const frame=encodePlayerActionFrame({kind,viewSeq:viewResult.sequence},this.actionSeq);this.transport.sendInput('action',frame);this.actionSeq=nextNetworkSequence(this.actionSeq);return{frame,view:viewResult};
  }
  ...
  sendHotbarSelect(slot){
    if(!this.isReady())return null;const frame=encodePlayerActionFrame({kind:'hotbar-select',slot},this.actionSeq);this.transport.sendInput('action',frame);this.actionSeq=nextNetworkSequence(this.actionSeq);return frame;
  }
}
