...
export class MultiplayerMovementSession{
  constructor({allowInsecure=false,bootstrapOptions={},bootstrapFactory=defaultBootstrapFactory,bridgeFactory=defaultBridgeFactory,interpolatorFactory=defaultInterpolatorFactory,onStateChange=()=>{},onReady=()=>{},onSnapshot=()=>{},onInventorySnapshot=()=>{},onRemotePlayerSpawn=()=>{},onRemotePlayerSnapshot=()=>{},onRemotePlayerDespawn=()=>{},onError=()=>{}}={}){
    ...
    this.bootstrap=null;this.bridge=null;this.interpolator=null;this.remotePlayerSystem=null;this.inventoryApplier=null;this.state='idle';this.latestControl=normalizeControlState();this.latestView={yaw:0,pitch:0};this.hasExplicitView=false;this.latestSnapshot=null;this.latestInventorySnapshot=null;this.readyData=null;this.remotePlayers=new Map();
  }
  ...
  resetRuntimeState(){this.detachRemotePlayerSystem();this.detachInventoryApplier();this.worldBlockBuffer.clear();this.bridge=null;this.interpolator=null;this.latestSnapshot=null;this.latestInventorySnapshot=null;this.readyData=null;this.latestControl=normalizeControlState();this.latestView={yaw:0,pitch:0};this.hasExplicitView=false;this.remotePlayers.clear();}
  ...
  close(code=1000,reason='multiplayer movement session closed'){const bootstrap=this.bootstrap,system=this.remotePlayerSystem;this.bootstrap=null;this.bridge=null;this.interpolator=null;this.remotePlayerSystem=null;this.inventoryApplier=null;this.worldBlockBuffer.clear();this.latestSnapshot=null;this.latestInventorySnapshot=null;this.readyData=null;this.remotePlayers.clear();if(bootstrap&&bootstrap.state!=='closed')bootstrap.close(code,reason);if(system)system.dispose();this.setState('closed',{code,reason});return true;}
}