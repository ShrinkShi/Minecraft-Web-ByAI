...
export class MultiplayerMovementSession{
  constructor({allowInsecure=false,bootstrapOptions={},bootstrapFactory=defaultBootstrapFactory,bridgeFactory=defaultBridgeFactory,interpolatorFactory=defaultInterpolatorFactory,onStateChange=()=>{},onReady=()=>{},onSnapshot=()=>{},onRemotePlayerSpawn=()=>{},onRemotePlayerSnapshot=()=>{},onRemotePlayerDespawn=()=>{},onError=()=>{}}={}){
    ...
    this.worldBlockBuffer=new OrderedChangeBuffer();if(!this.bootstrapOptions.clientFactory)this.bootstrapOptions.clientFactory=clientOptions=>new LiveWorldWebSocketClient({...clientOptions,onWorldBlockChange:change=>this.worldBlockBuffer.push(Object.freeze({...change}))});
  ...
  handleBootstrapReady(data){
    data=object(data,'multiplayer bootstrap ready data');const info=object(data.worldInfo,'multiplayer world info'),worldEdits=cloneWorldEdits(object(data.worldEdits,'initial authoritative world edits')),initial=cloneSnapshot(object(data.initialSnapshot,'initial authoritative player snapshot')),client=object(data.client,'multiplayer transport client');const interpolator=this.interpolatorFactory({tickRate:info.tickRate});if(!interpolator||typeof interpolator.accept!=='function'||typeof interpolator.step!=='function'||typeof interpolator.current!=='function')throw new TypeError('interpolatorFactory must return an AuthoritativePlayerInterpolator-compatible object');interpolator.accept(initial);this.interpolator=interpolator;this.latestSnapshot=cloneSnapshot(initial);if(!this.hasExplicitView)this.latestView={yaw:initial.yaw,pitch:initial.pitch};const bridge=this.bridgeFactory({transport:client,viewProvider:()=>({...this.latestView}),isReady:()=>this.bootstrap?.state==='ready'});if(!bridge||typeof bridge.prime!=='function'||typeof bridge.flush!=='function')throw new TypeError('bridgeFactory must return a MultiplayerInputBridge-compatible object');bridge.prime(this.latestControl,this.latestView);this.bridge=bridge;const initialFlush=bridge.flush();const ready=Object.freeze({client,worldInfo:info,worldEdits:Object.freeze({...worldEdits,edits:Object.freeze({...worldEdits.edits})}),initialSnapshot:cloneSnapshot(initial),initialDisplay:interpolator.current(),initialFlush});this.readyData=ready;this.onReady(ready);return ready;
  }
  ...
  handleRemotePlayerDespawn(message){const playerId=assertRemotePlayerId(message?.playerId);if(!this.remotePlayers.delete(playerId))throw new Error(`unknown remote player despawn in movement session: ${playerId}`);if(this.remotePlayerSystem)this.remotePlayerSystem.despawn(playerId);const copy={...message,playerId};this.onRemotePlayerDespawn(copy);return copy;}
