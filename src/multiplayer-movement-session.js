...
export class MultiplayerMovementSession{
  ...
  attachRemotePlayerSystem(system){
    system=remoteSystem(system);if(this.remotePlayerSystem)throw new Error('remote player system is already attached');this.remotePlayerSystem=system;
    try{for(const state of this.remotePlayers.values())system.spawn(cloneRemote(state));}catch(error){this.remotePlayerSystem=null;try{system.dispose();}catch{}throw error;}return system;
  }
