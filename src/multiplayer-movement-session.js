...
export class MultiplayerMovementSession{
  ...
  resetRuntimeState(){this.bridge=null;this.interpolator=null;this.latestSnapshot=null;this.readyData=null;this.latestControl=normalizeControlState();this.latestView={yaw:0,pitch:0};this.hasExplicitView=false;}
  ...
  connect(url){
    ...
    const bootstrap=this.bootstrapFactory({...this.bootstrapOptions,allowInsecure:this.allowInsecure,onStateChange:event=>this.handleBootstrapState(event),onReady:data=>this.handleBootstrapReady(data),onPlayerSnapshot:snapshot=>this.handleBootstrapSnapshot(snapshot),onError:error=>this.handleBootstrapError(error)});
    if(!bootstrap||typeof bootstrap.connect!=='function'||typeof bootstrap.close!=='function')throw new TypeError('bootstrapFactory must return a MultiplayerSessionBootstrap-compatible object');
    this.bootstrap=bootstrap;return bootstrap.connect(url);
  }
  ...
  handleBootstrapReady(data){
    data=object(data,'multiplayer bootstrap ready data');const info=object(data.worldInfo,'multiplayer world info'),initial=cloneSnapshot(object(data.initialSnapshot,'initial authoritative player snapshot')),client=object(data.client,'multiplayer transport client');
    const interpolator=this.interpolatorFactory({tickRate:info.tickRate});if(!interpolator||typeof interpolator.accept!=='function'||typeof interpolator.step!=='function'||typeof interpolator.current!=='function')throw new TypeError('interpolatorFactory must return an AuthoritativePlayerInterpolator-compatible object');
    interpolator.accept(initial);this.interpolator=interpolator;this.latestSnapshot=cloneSnapshot(initial);
    if(!this.hasExplicitView)this.latestView={yaw:initial.yaw,pitch:initial.pitch};
    const bridge=this.bridgeFactory({transport:client,viewProvider:()=>({...this.latestView}),isReady:()=>this.bootstrap?.state==='ready'});if(!bridge||typeof bridge.prime!=='function'||typeof bridge.flush!=='function')throw new TypeError('bridgeFactory must return a MultiplayerInputBridge-compatible object');
    bridge.prime(this.latestControl,this.latestView);this.bridge=bridge;const initialFlush=bridge.flush();
    const ready=Object.freeze({client,worldInfo:info,initialSnapshot:cloneSnapshot(initial),initialDisplay:interpolator.current(),initialFlush});this.readyData=ready;this.onReady(ready);return ready;
  }

  handleBootstrapSnapshot(snapshot){
    const copy=cloneSnapshot(snapshot);this.latestSnapshot=copy;if(!this.interpolator)return null;const result=this.interpolator.accept(copy),event={snapshot:cloneSnapshot(copy),result,display:this.interpolator.current()};this.onSnapshot(event);return event;
  }
  ...
  close(code=1000,reason='multiplayer movement session closed'){
    const bootstrap=this.bootstrap;this.bootstrap=null;this.bridge=null;this.interpolator=null;this.latestSnapshot=null;this.readyData=null;if(bootstrap&&bootstrap.state!=='closed')bootstrap.close(code,reason);this.setState('closed',{code,reason});return true;
  }
}
