...
export class MultiplayerMovementSession{
  ...
  close(code=1000,reason='multiplayer movement session closed'){const bootstrap=this.bootstrap,system=this.remotePlayerSystem;this.bootstrap=null;this.bridge=null;this.interpolator=null;this.remotePlayerSystem=null;this.inventoryApplier=null;this.itemEntityApplier=null;this.worldBlockBuffer.clear();this.latestSnapshot=null;this.latestInventorySnapshot=null;this.readyData=null;this.remotePlayers.clear();this.itemEntities.clear();if(bootstrap&&bootstrap.state!=='closed')bootstrap.close(code,reason);if(system)system.dispose();this.setState('closed',{code,reason});return true;}
}
