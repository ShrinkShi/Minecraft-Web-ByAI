...
import {FURNACE_SLOT,FURNACE_STACK_LIMIT,createFurnaceState,furnaceCanInsert,furnaceStackLimitFor,normalizeFurnaceStack,tickFurnace} from './smelting.js';
...
export class FurnaceContainerState{
...
export class FurnaceContainerHub{
  constructor(){this.states=new Map();}
  get furnaceCount(){return this.states.size;}
  has(target){return this.states.has(key(target));}
  open(target){const id=key(target);let state=this.states.get(id);if(!state){state=new FurnaceContainerState(target);this.states.set(id,state);}return state.snapshot();}
  ...
  restore(record){if(!record||typeof record!=='object'||Array.isArray(record))throw new TypeError('furnace restore record must be an object');const id=key(record.target);if(this.states.has(id))throw new Error(`furnace state already exists for cell: ${id}`);const state=new FurnaceContainerState(record.target,{state:record.state,revision:record.revision});this.states.set(id,state);return state.snapshot();}
