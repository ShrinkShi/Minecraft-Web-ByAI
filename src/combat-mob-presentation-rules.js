...
export function stepDaylightBurn(state,{dt,exposed,wet=false}={}){
  ...
  let remaining=Math.max(0,Number(state.remaining)||0),untilDamage=Math.max(0,Number(state.untilDamage)||0);const wasBurning=remaining>0;
  if(wet){remaining=0;untilDamage=0;}
  else if(exposed){if(!wasBurning&&untilDamage<=0)untilDamage=DAYLIGHT_BURN_INTERVAL_SECONDS;remaining=Math.max(remaining,DAYLIGHT_IGNITION_SECONDS);}
  else remaining=Math.max(0,remaining-dt);
  const burning=remaining>0;let damageEvents=0;
  if(burning){untilDamage-=dt;while(untilDamage<=0){damageEvents++;untilDamage+=DAYLIGHT_BURN_INTERVAL_SECONDS;}}
  else untilDamage=0;
...
export function creeperFuseVisual(progress,elapsedSeconds=0){
  const p=clamp01(progress),elapsed=Math.max(0,Number(elapsedSeconds)||0);const pulse=Math.sin(elapsed*(10+22*p));const scale=1+p*.2+Math.max(0,pulse)*p*.055;const whiteBase=p<=.34?0:Math.pow((p-.34)/.66,1.35),whitePulse=.35+.65*(.5+.5*Math.sin(elapsed*(8+30*p)));
