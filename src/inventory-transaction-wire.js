...
export const INVENTORY_TRANSACTION_PROTOCOL_VERSION=2;
...
export const INVENTORY_TRANSACTION_ACTIONS=Object.freeze(['slot-click','return-cursor','creative-pick']);
...
function itemId(value){if(typeof value!=='string'||!/^[a-z0-9:_-]{1,64}$/u.test(value))throw new RangeError('inventory creative-pick itemId must be 1 to 64 lowercase item id characters');return value;}
...
export function normalizeInventoryTransactionAction(value){
  ...
  if(type==='creative-pick'){exactKeys(value,['type','itemId'],'inventory creative-pick action');return Object.freeze({type,itemId:itemId(value.itemId)});}
...
export function decodeInventoryTransactionResult(raw,{expectedSession=null}={}){
  raw=object(raw,'inventory transaction result');exactKeys(raw,['v','kind','session','requestId','ok','code','revision'],'inventory transaction result');version(raw.v);if(raw.kind!==INVENTORY_TRANSACTION_RESULT_KIND)throw new RangeError(`inventory transaction result kind must be ${INVENTORY_TRANSACTION_RESULT_KIND}`);const session=assertClientSessionId(raw.session);if(expectedSession!==null&&session!==assertClientSessionId(expectedSession))throw new RangeError('inventory transaction request session mismatch');return encodeInventoryTransactionResult({session,requestId:raw.requestId,ok:raw.ok,code:raw.code,revision:raw.revision});
}
