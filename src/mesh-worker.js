...
function faceVisible(id,neighbor){
  ...
  const a=BLOCKS[id],b=BLOCKS[neighbor];
  if(a.liquid)return neighbor!==id;
...
self.onmessage=e=>{
  ...
  if(faceCount===0){
    self.postMessage({type:'mesh',key:m.key,cx:m.cx,cz:m.cz,version:m.version,empty:true});
    return;
  }
  ...
  self.postMessage({
    type:'mesh',key:m.key,cx:m.cx,cz:m.cz,version:m.version,empty:false,
