...
export async function createAuthoritativeMultiplayerGameplay({readyData,movement,scene,camera,canvas,controlState,onProgress=()=>{}}={}){
  ...
  let remotePlayers=null,releaseSecondary=null,inventoryAttached=false;
  try{
  ...
  }catch(error){if(inventoryAttached)movement.detachInventoryApplier?.();releaseSecondary?.();runtime.dispose();throw error;}
  const disposeRuntime=runtime.dispose.bind(runtime);let disposed=false;runtime.dispose=()=>{if(disposed)return;disposed=true;if(inventoryAttached){movement.detachInventoryApplier?.();inventoryAttached=false;}releaseSecondary?.();return disposeRuntime();};
