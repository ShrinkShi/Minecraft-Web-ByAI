...
export async function createAuthoritativeMultiplayerGameplay({readyData,movement,scene,camera,canvas,controlState,onProgress=()=>{}}={}){
  ...
  let remotePlayers=null,miningCracks=null,releaseSecondary=null,releaseCommandSender=null,inventoryAttached=false,itemEntitiesAttached=false;
  try{
    ...
    miningCracks=new MiningCrackOverlay(scene);remotePlayers=new RemotePlayerSystem(scene,{tickRate:info.tickRate});movement.attachRemotePlayerSystem(remotePlayers);releaseSecondary=installMultiplayerSecondaryRouting({runtime,movement});releaseCommandSender=attachMultiplayerCommandSender(text=>client.sendCommand(text));
  }catch(error){releaseCommandSender?.();miningCracks?.dispose();if(itemEntitiesAttached)movement.detachItemEntityApplier?.();if(inventoryAttached)movement.detachInventoryApplier?.();releaseSecondary?.();runtime.dispose();throw error;}
  const disposeRuntime=runtime.dispose.bind(runtime);let disposed=false;runtime.dispose=()=>{if(disposed)return;disposed=true;releaseCommandSender?.();releaseCommandSender=null;miningCracks?.dispose();miningCracks=null;if(itemEntitiesAttached){movement.detachItemEntityApplier?.();itemEntitiesAttached=false;}if(inventoryAttached){movement.detachInventoryApplier?.();inventoryAttached=false;}releaseSecondary?.();return disposeRuntime();};
