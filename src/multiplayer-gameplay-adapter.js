...
export async function createAuthoritativeMultiplayerGameplay({readyData,movement,scene,camera,canvas,controlState,onProgress=()=>{}}={}){
  ...
  let remotePlayers=null,releaseSecondary=null,inventoryAttached=false,itemEntitiesAttached=false;
  try{
    ...
    remotePlayers=new RemotePlayerSystem(scene,{tickRate:info.tickRate});movement.attachRemotePlayerSystem(remotePlayers);releaseSecondary=installMultiplayerSecondaryRouting({runtime,movement});
  }catch(error){if(itemEntitiesAttached)movement.detachItemEntityApplier?.();if(inventoryAttached)movement.detachInventoryApplier?.();releaseSecondary?.();runtime.dispose();throw error;}
  const disposeRuntime=runtime.dispose.bind(runtime);let disposed=false;runtime.dispose=()=>{if(disposed)return;disposed=true;if(itemEntitiesAttached){movement.detachItemEntityApplier?.();itemEntitiesAttached=false;}if(inventoryAttached){movement.detachInventoryApplier?.();inventoryAttached=false;}releaseSecondary?.();return disposeRuntime();};
