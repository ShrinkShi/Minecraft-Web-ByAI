...
export class PassiveMobSystem{
  ...
  spawn(type,position){
    const def=PASSIVE_MOBS[type];if(!def||this.store.size>=this.maxEntities)return null;const record=this.store.spawn(type,position,{hp:def.hp,hurtUntil:-Infinity,wanderAngle:Math.random()*Math.PI*2,wanderTimer:.5+Math.random()*2.5,fleeTimer:0,fleeX:position.x,fleeZ:position.z,pushX:0,pushZ:0,hurtPulse:0});const visual=bindMobVisual(this.templates.get(type).clone(true));visual.position.set(position.x,position.y,position.z);this.scene.add(visual);this.visuals.set(record.id,visual);return record;
  }
  ...
  tick(dt,player){
    this.spawnTimer-=dt;if(this.spawnTimer<=0){this.spawnTimer=1.35;this.trySpawnAround(player);}for(const record of[...this.store.values()]){const position=this.store.getPosition(record.id);if(!position)continue;const dx=position.x-player.position.x,dz=position.z-player.position.z;if(dx*dx+dz*dz>48*48){this.despawn(record.id);continue;}record.components.hurtPulse=Math.max(0,record.components.hurtPulse-dt);this.move(record,dt);}
  }
