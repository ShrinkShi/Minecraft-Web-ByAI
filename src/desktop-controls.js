...
export class DesktopControls{
  constructor(canvas,bus){
    this.canvas=canvas;this.bus=bus;this.source='desktop';this.gameplayEnabled=false;this.keys=new Set();this.bind();
  }

  bind(){
    this.onKeyDown=e=>{
      const code=e.code;
      if((MOVEMENT_CODES.has(code)||BUTTON_CODES.has(code))&&this.gameplayEnabled){this.keys.add(code);this.syncContinuous();if(code==='Space')e.preventDefault();}
      if(e.repeat)return;
      let handled=false;
    ...
    this.onKeyUp=e=>{if(MOVEMENT_CODES.has(e.code)||BUTTON_CODES.has(e.code)){this.keys.delete(e.code);this.syncContinuous();}};
  ...
  syncContinuous(){
    ...
    for(const name of ['jump','sneak','sprint']){let pressed=false;for(const [code,button] of BUTTON_CODES)if(button===name&&this.keys.has(code)){pressed=true;break;}this.bus.setButton(this.source,name,pressed);}
  }

  reset(){this.keys.clear();this.bus.resetSource(this.source);}
