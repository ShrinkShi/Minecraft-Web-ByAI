...
import {applyHudModePresentation} from './hud-presentation-runtime.js';
...
function renderPlayerStatus(){if(!player)return;applyHudModePresentation(ui,player.mode);const xp=experienceState(totalXp);ui.renderStatus(player.hp,player.hunger,xp.progress*100,xp.level,equipment?.armorPoints()||0);}
...
function handleControlIntent({name,payload}={}){
  ...
  if(name==='drop'){if(sessionKind==='multiplayer')return multiplayerUnsupported('联机丢弃/物品权威尚未接入');if(!canControl())return false;singleplayerFoodUse?.cancel('drop');dropSelected();return true;}
...
function updateSurvival(dt){if(player.mode!=='survival')return;const result=player.stepHunger(dt);if(result.changed)markSaveDirty();renderPlayerStatus();}
...
function updateMultiplayer(dt){
  const movement=multiplayerMovement;if(!movement?.ready||!world||!player)return;movement.flush();const authoritative=movement.step(dt);if(authoritative){applyAuthoritativePlayerState(player,authoritative);if(worldInfo)worldInfo.mode=authoritative.mode;applyHudModePresentation(ui,player.mode);}world.ensureAround(player.position.x,player.position.z);selectedTarget=null;ui.setBreak(0);ui.renderOxygen(MAX_AIR_SECONDS,MAX_AIR_SECONDS,false);
