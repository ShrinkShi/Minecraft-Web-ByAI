...
async function startWorld(){
  ...
  gameplayRuntime=await createClientGameplayRuntime({scene,camera,canvas,seed:worldInfo.seed,prompt:worldInfo.prompt,terrainVersion:worldInfo.terrainVersion,renderDistance:3,savedEdits:saved?.edits||{},centerX,centerZ,mode,inventoryState:saved?.inventory||null,equipmentState:saved?.equipment||null,controlState:controlBus.snapshot(),weather,onWorldEdit:event=>{markSaveDirty();singleplayerFarming?.observeEdit(event);},onWorldProgress:(done,total)=>ui.showLoading(true,`生成区块 ${done}/${total}`,total?Math.round(done/total*100):100),onInventoryPickup:()=>{ui.refreshInventory();markSaveDirty();},onExperience:addExperience,onPlayerHit:handlePlayerHit,onPlayerBlast:handlePlayerBlast,onMobDeath:handleMobDeath,onHostileProjectile:handleHostileProjectile,onHostileExplosion:handleHostileExplosion});
