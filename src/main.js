...
let world=null,player=null,inventory=null,drops=null,running=false,paused=false,last=performance.now(),selectedTarget=null,breakStart=0,lastSecond=0,frames=0,fps=0,worldInfo=null;
...
function disposeWorld(){
  ...
  drops?.dispose();player?.dispose();world?.dispose();drops=null;player=null;world=null;inventory=null;worldInfo=null;saveDirty=false;lastSavedPosition=null;
}
...
function spawnOverflow(stacks){
  if(!drops||!player)return;const origin=player.position.clone().add(new THREE.Vector3(0,1,0));
  for(const stack of stacks)drops.spawn(stack.id,stack.count,origin.clone());
}

async function startWorld(){
  disposeWorld();
  const seed=document.querySelector('#world-seed').value||String(Date.now()),prompt=document.querySelector('#terrain-prompt').value,selectedMode=document.querySelector('#game-mode').value,name=document.querySelector('#world-name').value||'新的世界',id=worldIdFor(name,seed);
  let saved=null;try{saved=await storage.getWorld(id);}catch(error){console.warn('无法读取 IndexedDB 存档，将启动无持久化会话',error);}
  const mode=saved?.mode||selectedMode;worldInfo={id,seed:saved?.seed||seed,prompt:saved?.prompt||prompt,mode,name:saved?.name||name};gameTime=Number.isFinite(saved?.gameTime)?saved.gameTime:6000;weather=saved?.weather||'clear';
  ...
  player=new PlayerController(camera,canvas,world,scene);player.setMode(mode);const restored=saved?.player?player.restore(saved.player):false;if(!restored)player.spawn(centerX,centerZ);
  ...
  running=true;paused=false;saveDirty=!saved;lastSavedPosition=player.position.clone();ui.hud.classList.remove('hidden');ui.showLoading(false);ui.renderStatus(player.hp,player.hunger,0,0);applySky();
  ui.chatMessage('按 T 聊天，按 / 直接输入指令；F5 切换视角。');ui.showToast(saved?'已从浏览器存档恢复世界':mode==='creative'?'创造模式':'生存模式');pointer();
}
...
async function handleAction(action){
  if(action==='singleplayer')modeScreen('world');else if(action==='back-main')modeScreen('main');else if(action==='create-world')await startWorld();else if(action==='resume')resume();
  ...
  else if(action==='options')ui.showToast('选项系统将在后续阶段接入');else if(action==='multiplayer'||action==='realms')ui.showToast('网络层将在后续阶段接入');else if(action==='quit')ui.showToast('浏览器页面不能由网页强制关闭');
}

document.addEventListener('click',e=>{const button=e.target.closest('[data-action]');if(button)handleAction(button.dataset.action);});canvas.addEventListener('click',pointer);
...
ui.chatInput.addEventListener('keydown',e=>{
  ...
  if(e.key==='Enter'){
    e.preventDefault();const text=ui.chatInput.value;ui.closeChat();if(text.trim())runCommand(text);pointer();
  }
});
...
canvas.addEventListener('mousedown',e=>{
  ...
  if(e.button===0){if(player.mode!=='spectator'&&player.mode!=='adventure')breakStart=performance.now();}
  if(e.button===2){
...
function interaction(now){
  ...
  const block=BLOCKS[selectedTarget.id],hardness=block?.hardness||1,creative=player.mode==='creative',duration=creative?70:Math.max(120,hardness*900/toolMultiplier(selectedTarget.id)),progress=(now-breakStart)/duration;ui.setBreak(Math.min(1,progress));
  if(progress>=1){
    const broken={...selectedTarget};if(world.setBlock(broken.x,broken.y,broken.z,0)){
...
function dropSelected(){
  ...
  const dir=player.lookDirection(new THREE.Vector3()),origin=player.eyePosition(new THREE.Vector3()).addScaledVector(dir,.55),velocity=dir.clone().multiplyScalar(4).add(new THREE.Vector3(0,1.5,0));drops.spawn(stack.id,stack.count,origin,velocity);markSaveDirty();
}
...
function animate(now){
  ...
  if(running&&!paused&&player){
    if(!ui.hasOpenPanel()&&!ui.isChatOpen())player.update(dt);world.ensureAround(player.position.x,player.position.z);interaction(now);updateSurvival(dt);drops?.update(dt,player);updateAutosave(now);gameTime=(gameTime+dt*20)%24000;applySky();
    const p=player.position;ui.debug.textContent=`Minecraft Web By AI v0.3.0\nFPS ${fps} · WebGL ${renderer.capabilities.isWebGL2?'2':'1'}\nXYZ ${p.x.toFixed(1)} / ${p.y.toFixed(1)} / ${p.z.toFixed(1)}\nChunks ${world.chunks.size} · Meshes ${world.meshes.size} · MeshQ ${world.meshQueue.size}\nDrops ${drops?.drops.length||0} · Time ${Math.floor(gameTime)} · ${weather}\n模式 ${player.mode} · Seed ${worldInfo?.seed}`;
  }
