...
function disposeWorld(){
  running=false;clearPlayerInput();const movement=multiplayerMovement;multiplayerMovement=null;multiplayerStarting=false;singleplayerVegetation?.dispose();singleplayerVegetation=null;singleplayerFarming?.dispose();singleplayerFarming=null;singleplayerFurnace?.dispose();singleplayerFurnace=null;sessionKind=null;try{movement?.close(1000,'leaving multiplayer world');}catch(error){console.warn('关闭多人连接失败',error);}deathState=null;deathScreen.hide();document.exitPointerLock?.();ui.closeChat();ui.inventory.classList.add('hidden');ui.workbench.classList.add('hidden');resetOxygen();ui.setBreak(0);
