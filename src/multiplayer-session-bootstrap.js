...
export class MultiplayerSessionBootstrap{
  ...
  armSyncTimer(){this.clearSyncTimer();this.syncTimer=this.setTimer(()=>{this.syncTimer=null;if(this.state==='synchronizing')this.fail(new Error('multiplayer world synchronization timed out'),WORLD_SYNC_TIMEOUT_CLOSE_CODE,'world sync timeout');},this.worldSyncTimeoutMs);}
  ...
  connect(url){
    ...
    const client=this.clientFactory({allowInsecure:this.allowInsecure,onStateChange:event=>this.handleTransportState(event),onProtocolError:error=>this.handleProtocolError(error),onWorldInfo:info=>this.handleWorldInfo(info),onPlayerSnapshot:snapshot=>this.handlePlayerSnapshot(snapshot),onRemotePlayerSpawn:message=>this.onRemotePlayerSpawn(message),onRemotePlayerSnapshot:message=>this.onRemotePlayerSnapshot(message),onRemotePlayerDespawn:message=>this.onRemotePlayerDespawn(message)});
    ...
    this.client=client;this.setState('connecting');try{return client.connect(url);}catch(error){this.fail(error,null,null,{closeTransport:false});throw error;}
  }
  ...
  handleTransportState({state,detail}={}){
    ...
    if(state==='closed'){this.clearSyncTimer();if(this.state!=='failed'){this.worldInfo=null;this.latestSnapshot=null;this.readyData=null;this.setState('closed',detail);}return;}
  }

  handleProtocolError(error){if(this.state!=='failed')this.fail(error,null,null,{closeTransport:false});}
  handleWorldInfo(info){this.worldInfo=cloneInfo(info);this.onWorldInfo(this.worldInfo);this.maybeReady();}
  handlePlayerSnapshot(snapshot){this.latestSnapshot=cloneSnapshot(snapshot);this.onPlayerSnapshot(this.latestSnapshot);this.maybeReady();}
  maybeReady(){
    if(this.readyData||!this.worldInfo||!this.latestSnapshot)return null;if(!this.client||this.client.state!=='ready')return null;
  ...
  fail(error,closeCode=WORLD_SYNC_TIMEOUT_CLOSE_CODE,reason='multiplayer bootstrap failed',{closeTransport=true}={}){const failure=error instanceof Error?error:new Error(String(error));this.clearSyncTimer();this.lastError=failure;this.worldInfo=null;this.latestSnapshot=null;this.readyData=null;this.setState('failed',failure.message);try{this.onError(failure);}catch{}if(closeTransport&&closeCode!==null&&this.client&&this.client.state!=='closed'&&this.client.state!=='error')try{this.client.close(closeCode,reason);}catch{}return failure;}
  close(code=1000,reason='multiplayer bootstrap closed'){this.clearSyncTimer();const client=this.client;this.client=null;this.worldInfo=null;this.latestSnapshot=null;this.readyData=null;this.lastError=null;if(client&&client.state!=='closed')client.close(code,reason);this.setState('closed',{code,reason});}
}
