...
export function inspectBlockTarget(blockId,{selectedItemId=null}={}){
  ...
  const requiredTool=block.requires||null,requiredTier=minimumToolTier(block),tool=selectedTool(selectedItemId),hasRequiredTool=toolMeetsBlockRequirement(tool,block);
  ...
  return Object.freeze({
    ...
    requiredToolTier:requiredTier,requiredToolTierName:requiredTier?toolTierLabel(requiredTier):null,
    heldTool:tool?.kind||null,heldToolTier:tool?.tier||null,heldToolTierName:tool?.tier?toolTierLabel(tool.tier):null,toolCorrect:hasRequiredTool,
