...
export function installMultiplayerCombatPresentation({movement,runtime,canvas}={}){
  ...
  const release=subscribeMultiplayerCombatSnapshots(present),capabilityObserver=new MutationObserver(()=>upgradeCapabilityNotice());capabilityObserver.observe(chatLog,{childList:true,subtree:true});const current=currentMultiplayerCombatSnapshot();if(current)present(current);upgradeCapabilityNotice();
  ...
  return()=>{if(!active)return false;active=false;cancelFinish();capabilityObserver.disconnect();release();document.removeEventListener('click',onClick,true);deathRoot.classList.remove('active');if(lastSnapshot)renderHealth(hearts,lastSnapshot.maxHp,lastSnapshot.maxHp);return true;};
}
