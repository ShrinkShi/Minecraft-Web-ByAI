...
export const MULTIPLAYER_PVP_CAPABILITY_NOTICE='多人服务器已接管移动、普通方块交互、掉落物、背包、装备、玩家 2×2 合成与工作台 3×3 容器，以及玩家 PvP 近战；怪物战斗、投射物、爆炸和本地存档仍未接入。';
...
export function installMultiplayerCombatPresentation({movement,runtime,canvas}={}){
  ...
  const deathRoot=document.querySelector('#death-menu'),reason=document.querySelector('#death-reason'),detail=document.querySelector('#death-detail'),hearts=document.querySelector('#hearts'),chatLog=document.querySelector('#chat-log');if(!deathRoot||!reason||!detail||!hearts||!chatLog)throw new Error('multiplayer combat presentation DOM is incomplete');
  ...
  const release=subscribeMultiplayerCombatSnapshots(present),releaseDeadActions=registerControlActionInterceptor(()=>dead?false:undefined),capabilityObserver=new MutationObserver(()=>upgradeCapabilityNotice());capabilityObserver.observe(chatLog,{childList:true,subtree:true});const current=currentMultiplayerCombatSnapshot();if(current)present(current);upgradeCapabilityNotice();
  ...
  return()=>{if(!active)return false;active=false;cancelFinish();capabilityObserver.disconnect();releaseDeadActions();release();document.removeEventListener('click',onClick,true);deathRoot.classList.remove('active');if(lastSnapshot)renderHealth(hearts,lastSnapshot.maxHp,lastSnapshot.maxHp);return true;};
}
