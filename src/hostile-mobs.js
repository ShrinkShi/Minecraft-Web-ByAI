...
import {currentDifficulty,hostileSpawningAllowed,scaleDifficultyDamage} from './game-settings.js';
import {retaliationTargetId} from './mob-retaliation-rules.js';
...
export class HostileMobSystem{
  constructor(scene,world,{maxEntities=8,cellSize=8,onPlayerHit=()=>{},onProjectile=()=>{},onExplosion=()=>{},onDeath=()=>{},onBurn=()=>{},onFuseStart=()=>{},onSound=()=>{},getEnvironment=()=>({weather:'clear'}),getDifficulty=currentDifficulty}={}){
    this.scene=scene;this.world=world;this.maxEntities=maxEntities;this.onPlayerHit=onPlayerHit;this.onProjectile=onProjectile;this.onExplosion=onExplosion;this.onDeath=onDeath;this.onBurn=onBurn;this.onFuseStart=onFuseStart;this.onSound=typeof onSound==='function'?onSound:()=>{};this.getEnvironment=typeof getEnvironment==='function'?getEnvironment:()=>({weather:'clear'});this.getDifficulty=typeof getDifficulty==='function'?getDifficulty:currentDifficulty;this.store=new EntityStore({cellSize});this.visuals=new Map();this.spawnTimer=.7;this.aiAccumulator=0;this.simulationTimeMs=0;
  ...
  spawn(type,position){
    const def=HOSTILE_MOBS[type];if(!def||this.store.size>=this.maxEntities||!hostileSpawningAllowed(this.getDifficulty()))return null;
    const record=this.store.spawn(type,position,{hp:def.hp,hurtUntil:-Infinity,attackTimer:.3+Math.random()*.5,pushX:0,pushZ:0,hurtPulse:0,strafeDir:Math.random()<.5?-1:1,strafeTimer:1+Math.random()*1.5,fuse:0,fuseWasActive:false,burnRemaining:0,burnDamageTimer:0,burning:false,ambientTimer:2+Math.random()*8,retaliationTargetId:null});
  ...
  hurt(record,amount,sourcePosition,now,{attackerEntity=null}={}){
    ...
    const retaliation=retaliationTargetId({victimType:record.type,attackerType:attackerEntity?.type,attackerId:attackerEntity?.id});if(retaliation&&this.store.has(retaliation))record.components.retaliationTargetId=retaliation;
  ...
  updateFuse(record,def,state,position,planar,vertical,dt){
    ...
    const event={position:{x:position.x,y:position.y+def.height*.45,z:position.z},radius:def.explosionRadius,damageRadius:def.damageRadius,maxDamage:scaleDifficultyDamage(def.maxDamage,this.getDifficulty()),entity:record};this.despawn(record.id);this.onExplosion(event);return true;
  }

  attack(record,def,state,position,target,planar,vertical){
    if(def.attackStyle==='fuse'||state.attackTimer>0||vertical>=5||planar>def.followRange)return;
    const damage=scaleDifficultyDamage(def.attackDamage,this.getDifficulty());if(damage<=0)return;
    if(def.attackStyle==='ranged'){
      state.attackTimer=def.attackCooldown;const invPlanar=1/Math.max(planar,.001),fx=(target.position.x-position.x)*invPlanar,fz=(target.position.z-position.z)*invPlanar,rx=-fz,rz=fx;
      this.onProjectile({kind:'arrow',damage,speed:def.projectileSpeed,source:{x:position.x+rx*.28+fx*.11,y:position.y+def.height*.68,z:position.z+rz*.28+fz*.11,entityId:record.id,entityType:record.type},target:{x:target.position.x,y:target.position.y+(Number(target.eye)||1.35)*.8,z:target.position.z},entity:record});return;
    }
    if(planar<=def.attackRange&&vertical<1.7){state.attackTimer=def.attackCooldown;if(target.entityRecord)this.hurt(target.entityRecord,damage,{x:position.x,z:position.z},this.simulationTimeMs,{attackerEntity:record});else this.onPlayerHit({amount:damage,source:{x:position.x,z:position.z},entity:record});}
  }

  retaliationTarget(record){
    const id=record?.components?.retaliationTargetId;if(!Number.isInteger(id)||id<=0)return null;const entity=this.store.get(id),position=this.store.getPosition(id);if(!entity||!position||entity.id===record.id){record.components.retaliationTargetId=null;return null;}const def=HOSTILE_MOBS[entity.type];return{mode:'survival',position,eye:(def?.height||1.8)*.75,entityRecord:entity};
  }

  moveAndAttack(record,dt,target){
    ...
    const targetable=!!target?.entityRecord||canHostileMobTargetPlayer(target);let toX=0,toZ=0,planar=Infinity,vertical=Infinity,vx=state.pushX,vz=state.pushZ;
    if(targetable){toX=target.position.x-position.x;toZ=target.position.z-position.z;planar=Math.hypot(toX,toZ);vertical=Math.abs(target.position.y-position.y);if(this.updateFuse(record,def,state,position,planar,vertical,dt))return;({vx,vz}=this.desiredVelocity(def,state,position,target,planar,vertical,dt));}
