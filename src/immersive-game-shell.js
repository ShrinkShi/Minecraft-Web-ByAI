...
function itemIdFromHotbar(hotbar){
  const selected=hotbar?.querySelector?.('.hotbar-slot.selected');if(!selected)return null;const name=String(selected.title||'').split('\n')[0];if(!name)return null;for(const[id,def]of Object.entries(ITEMS))if(def?.name===name)return id;return null;
}
