...
static func hash_terrain_seed(value: String = "1") -> int:
	var text := value if not value.is_empty() else "1"
	var hash := FNV_OFFSET
...
func hash2(x: int, z: int) -> float:
	var hash := (_imul32(x, 374761393) ^ _imul32(z, 668265263) ^ seed_hash) & UINT32_MASK
	var mixed_signed := _i32((hash ^ (hash >> 13)) & UINT32_MASK)
	var multiplied := float(mixed_signed) * 1274126177.0
	var multiplied_bits := _number_to_uint32(multiplied)
	var output := (multiplied_bits ^ (multiplied_bits >> 16)) & UINT32_MASK
	return float(output) / 4294967295.0

func hash3(x: int, y: int, z: int, salt: int = 0) -> float:
	var hash := (seed_hash ^ _imul32(x, 374761393) ^ _imul32(y, 668265263) ^ _imul32(z, 1274126177) ^ _imul32(salt, 1597334677)) & UINT32_MASK
	hash = _imul32((hash ^ (hash >> 13)) & UINT32_MASK, 1274126177)
	var output := (hash ^ (hash >> 16)) & UINT32_MASK
	return float(output) / 4294967295.0
...
func height_at(x: int, z: int) -> int:
	var continental := (fbm(float(x) * 0.55, float(z) * 0.55) - 0.5) * float(parameters["amp"])
	var detail := (fbm(float(x) + 731.0, float(z) - 271.0) - 0.5) * 4.0
...
func is_iron_ore(x: int, y: int, z: int, top: int = IRON_MAX_Y + 4) -> bool:
	var maximum_y := mini(IRON_MAX_Y, top - 4)
	if y < IRON_MIN_Y or y > maximum_y:
		return false
	var vein := hash3(_floor_div(x, IRON_VEIN_CELL), _floor_div(y, IRON_VEIN_CELL), _floor_div(z, IRON_VEIN_CELL), IRON_VEIN_SALT)
...
func is_coal_ore(x: int, y: int, z: int, top: int = COAL_MAX_Y + 4) -> bool:
	if version < 3:
		return false
	var maximum_y := mini(COAL_MAX_Y, top - 4)
	if y < COAL_MIN_Y or y > maximum_y:
		return false
	var vein := hash3(_floor_div(x, COAL_VEIN_CELL), _floor_div(y, COAL_VEIN_CELL), _floor_div(z, COAL_VEIN_CELL), COAL_VEIN_SALT)
...
func generate_chunk(cx: int, cz: int) -> PackedByteArray:
	...
	for lx in range(CHUNK_SIZE):
		for lz in range(CHUNK_SIZE):
			...
			var moisture := fbm(float(wx) + 2000.0, float(wz) - 900.0)
			var sandy := top <= int(parameters["sea"]) + 1 or moisture < float(parameters["sand"])
	...
	for y in range(base + 2, base + 6):
		for x in range(lx - 2, lx + 3):
			for z in range(lz - 2, lz + 3):
				var distance := absi(x - lx) + absi(z - lz) + (1 if y == base + 5 else 0)
...
static func _imul32(a: int, b: int) -> int:
	var left := a & UINT32_MASK
	var right := b & UINT32_MASK
