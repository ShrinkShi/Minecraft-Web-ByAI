...
function blockId(value){if(!Number.isInteger(value)||value<0||value>=BLOCKS.length||!BLOCKS[value])throw new RangeError('block id must reference a known block');return value;}
...
export class ServerTerrainWorld{
  ...
  getChunkSnapshot(cx,cz){
    cx=integer(cx,'cx');cz=integer(cz,'cz');const snapshot=this.#getChunk(cx,cz).slice();
    if(!this.#edits.size)return snapshot;
    for(const[key,id]of this.#edits){const{x,y,z}=parseEditKey(key);if(floorDiv(x,CHUNK_SIZE)!==cx||floorDiv(z,CHUNK_SIZE)!==cz)continue;snapshot[terrainChunkIndex(mod(x,CHUNK_SIZE),y,mod(z,CHUNK_SIZE))]=id;}
    return snapshot;
  }
  ...
  getBlock(wx,wy,wz){
    wx=integer(wx,'wx');wy=integer(wy,'wy');wz=integer(wz,'wz');if(wy<0)return BLOCK.STONE;if(wy>=WORLD_HEIGHT)return BLOCK.AIR;const edited=this.#edits.get(editKey(wx,wy,wz));return edited===undefined?this.getBaseBlock(wx,wy,wz):edited;
  }

  setBlock(wx,wy,wz,id){
    wx=integer(wx,'wx');wy=editableY(wy);wz=integer(wz,'wz');id=blockId(id);const previous=this.getBlock(wx,wy,wz),base=this.getBaseBlock(wx,wy,wz);if(previous===id)return Object.freeze({changed:false,revision:this.revision,x:wx,y:wy,z:wz,previous,id,base,storedEdit:this.#edits.has(editKey(wx,wy,wz))});
    const key=editKey(wx,wy,wz);if(id===base)this.#edits.delete(key);else this.#edits.set(key,id);this.revision=(this.revision+1)>>>0;return Object.freeze({changed:true,revision:this.revision,x:wx,y:wy,z:wz,previous,id,base,storedEdit:this.#edits.has(key)});
  }

  editEntries(){
    const entries=[];for(const[key,id]of this.#edits){const{x,y,z}=parseEditKey(key);entries.push({x,y,z,id});}entries.sort((a,b)=>a.x-b.x||a.z-b.z||a.y-b.y||a.id-b.id);return entries.map(Object.freeze);
  }
  ...
  highestSolid(x,z){
    const wx=Math.floor(finite(x,'x')),wz=Math.floor(finite(z,'z'));for(let y=WORLD_HEIGHT-1;y>=0;y--)if(this.isSolidBlock(wx,y,wz))return y;return 0;
  }
}
