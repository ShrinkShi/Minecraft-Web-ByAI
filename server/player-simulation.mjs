...
import {normalizeFlyingForMode,toggleCreativeFlightState} from '../src/creative-flight-rules.js';
...
const MODE_SET=new Set(SERVER_PLAYER_MODES),MAX_UINT32=0xffffffff;
...
function optionalSequence(value){if(value===undefined||value===null)return null;if(!Number.isInteger(value)||value<0||value>MAX_UINT32)throw new RangeError('flight toggle sequence must be uint32 or null');return value;}
...
function normalizedInput(inputState,session){if(inputState===null||inputState===undefined)return{control:normalizeControlState(),view:null,flightToggleSequence:null};if(typeof inputState!=='object'||Array.isArray(inputState))throw new TypeError('inputState must be an object');if(inputState.session!==undefined&&inputState.session!==session)throw new RangeError('inputState session does not match simulation session');const control=inputState.control?normalizeControlState(inputState.control):normalizeControlState();let view=null;if(inputState.view){if(typeof inputState.view!=='object'||Array.isArray(inputState.view))throw new TypeError('inputState.view must be an object');view={yaw:normalizePlayerYaw(inputState.view.yaw),pitch:pitch(inputState.view.pitch)};}return{control,view,flightToggleSequence:optionalSequence(inputState.flightToggleSequence)};}
...
export class ServerPlayerSimulation{
  ...
  addSession(session,{position,velocity={x:0,y:0,z:0},yaw=0,pitch:initialPitch=0,mode:initialMode='survival'}={}){session=assertClientSessionId(session);if(this.players.has(session))throw new Error(`server player session already exists: ${session}`);const playerMode=mode(initialMode),state={session,tick:0,position:vector3(position,'position'),velocity:vector3(velocity,'velocity'),yaw:normalizePlayerYaw(yaw),pitch:pitch(initialPitch),mode:playerMode,flying:normalizeFlyingForMode(playerMode,false),lastFlightToggleSequence:null,grounded:false,swimCoverage:0,voided:false};this.players.set(session,state);return cloneState(state);}
  ...
  setMode(session,nextMode){session=assertClientSessionId(session);const state=this.players.get(session);if(!state)throw new Error(`unknown server player session: ${session}`);const previous=state.mode;state.mode=mode(nextMode);state.flying=normalizeFlyingForMode(state.mode,previous===state.mode?state.flying:false);if(state.flying)state.swimCoverage=0;return cloneState(state);}
  setFlying(session,flying){session=assertClientSessionId(session);const state=this.players.get(session);if(!state)throw new Error(`unknown server player session: ${session}`);state.flying=normalizeFlyingForMode(state.mode,flying);if(state.flying){state.swimCoverage=0;state.velocity.y=0;}return cloneState(state);}
  toggleFlight(session){session=assertClientSessionId(session);const state=this.players.get(session);if(!state)throw new Error(`unknown server player session: ${session}`);const result=toggleCreativeFlightState(state.mode,state.flying);if(result.changed)this.setFlying(session,result.flying);return Object.freeze({...result,snapshot:cloneState(state)});}
