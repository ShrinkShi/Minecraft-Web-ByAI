...
export function createAuthoritativeServerRuntime({config={},setIntervalFn=setInterval,clearIntervalFn=clearInterval,playerIdFactory=undefined,onLog=()=>{},onError=()=>{}}={}){
  ...
  authoritative=new AuthoritativeWorldSession({world,getInputState:session=>server?.getSessionInputState(session)??null,sendPlayerSnapshot:(session,snapshot)=>{const wire=server?.sendPlayerSnapshot(session,snapshot)??null;if(wire!==null){try{creativeBreak.step(session,snapshot,server?.getSessionInputState(session)??null);}catch(error){report({source:'creative-block-break',session,error});}replication.update(session,snapshot);}return wire;},setIntervalFn,clearIntervalFn,onSessionError:event=>report({source:'world-session',...event}),defaultMode:normalized.mode,spawnX:normalized.spawnX,spawnZ:normalized.spawnZ,prefetchRadius:normalized.prefetchRadius});
  ...
  },onSessionClose:({session})=>{if(session){creativeBreak.remove(session);replication.leave(session);authoritative.leave(session);}},onSocketError:event=>report({source:'socket',...event})});
