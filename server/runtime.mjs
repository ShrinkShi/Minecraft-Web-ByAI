...
export function createAuthoritativeServerRuntime({
  ...
  let server=null;
  const replication=new RemotePlayerReplicationHub({
    sendSpawn:(session,state)=>server?.sendRemotePlayerSpawn(session,state)??null,
    sendSnapshot:(session,state)=>server?.sendRemotePlayerSnapshot(session,state)??null,
    sendDespawn:(session,playerId)=>server?.sendRemotePlayerDespawn(session,playerId)??null,
    ...(playerIdFactory===undefined?{}:{playerIdFactory:callback(playerIdFactory,'playerIdFactory')}),
    onSendError:event=>report({source:'remote-replication',...event})
  });
  const authoritative=new AuthoritativeWorldSession({
    ...
    sendPlayerSnapshot:(session,snapshot)=>{
      const wire=server?.sendPlayerSnapshot(session,snapshot)??null;if(wire!==null)replication.update(session,snapshot);return wire;
    },
    setIntervalFn,
    clearIntervalFn,
  ...
  server=createMultiplayerServer({
    ...
    onSessionReady:({session})=>{
      ...
      const joined=authoritative.join(session,{mode:normalized.mode});
      try{replication.join(session,joined.snapshot);}catch(error){authoritative.leave(session);throw error;}
    },
    onSessionClose:({session})=>{if(session){replication.leave(session);authoritative.leave(session);}},
    onSocketError:event=>report({source:'socket',...event})
  });

  let state='idle',address=null,stopPromise=null;
  const runtime={
    config:normalized,world,server,authoritative,replication,
    ...
    async start(){
      ...
      try{
        ...
        state='failed';replication.close();authoritative.close();try{await server.close();}catch{}report({source:'runtime-start',error});throw error;
      }
    },
    async stop(){
      ...
      stopPromise=(async()=>{
        const wasRunning=state==='running'||state==='starting'||state==='failed';state='stopping';replication.close();authoritative.close();
