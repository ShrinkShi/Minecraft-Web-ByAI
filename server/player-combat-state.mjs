...
export class ServerPlayerCombatState{
  ...
  tryAttack(nowMs,playerMode){nowMs=finite(nowMs,'attack time');playerMode=mode(playerMode);if(this.dead)return Object.freeze({accepted:false,reason:'attacker-dead'});if(playerMode==='spectator')return Object.freeze({accepted:false,reason:'attacker-spectator'});if(!canAttack(this.lastAttackAt,nowMs,this.attackCooldownMs))return Object.freeze({accepted:false,reason:'attack-cooldown'});this.lastAttackAt=nowMs;return Object.freeze({accepted:true,reason:'attack-ready'});}
...
export class ServerPlayerCombatHub{
  ...
  tryAttack(session,nowMs,modeValue){return this.state(session).tryAttack(nowMs,modeValue);}
