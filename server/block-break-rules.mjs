...
function targetLike(value){if(!value||typeof value!=='object'||Array.isArray(value))throw new TypeError('block target must be an object');return{x:integer(value.x,'target.x'),y:integer(value.y,'target.y'),z:integer(value.z,'target.z'),id:blockId(value.id,'target.id')};}
