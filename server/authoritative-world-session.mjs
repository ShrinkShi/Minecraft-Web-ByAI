...
function worldLike(value){
  if(!value||typeof value!=='object'||Array.isArray(value))throw new TypeError('world must be an object');
  if(!value.environment||typeof value.environment!=='object')throw new TypeError('world.environment must be an object');
  for(const name of ['highestSolid','prefetchAround'])if(typeof value[name]!=='function')throw new TypeError(`world.${name} must be a function`);
  return value;
}
function simulationLike(value){
  if(!value||typeof value!=='object'||Array.isArray(value))throw new TypeError('simulation must be an object');
  for(const name of ['addSession','removeSession','hasSession','snapshot','step','setMode'])if(typeof value[name]!=='function')throw new TypeError(`simulation.${name} must be a function`);
  return value;
}
...
export class AuthoritativeWorldSession{
  constructor({
    ...
    prefetchRadius:initialPrefetchRadius=DEFAULT_AUTHORITATIVE_PREFETCH_RADIUS
  }={}){
  ...
  spawnPosition(x=this.spawnX,z=this.spawnZ,prefetch=this.prefetchRadius){
    x=finite(x,'spawnX');z=finite(z,'spawnZ');prefetch=prefetchRadius(prefetch);this.world.prefetchAround(x,z,prefetch);const blockX=Math.floor(x),blockZ=Math.floor(z),ground=this.world.highestSolid(blockX,blockZ);
    return{x:blockX+.5,y:ground+1.001,z:blockZ+.5,ground};
  }
  ...
  tickSession(session){
    session=assertClientSessionId(session);if(!this.sessions.has(session))return{session,stepped:false,sent:false,reason:'not-joined'};
    let inputState;try{inputState=this.getInputState(session);}catch(error){this.reportSessionError(session,error,'input');this.leave(session);return{session,stepped:false,sent:false,reason:'input-error'};}
    if(inputState===null||inputState===undefined){return{session,stepped:false,sent:false,reason:'input-unavailable'};}
    let snapshot;try{snapshot=this.simulation.step(session,inputState);}catch(error){this.reportSessionError(session,error,'simulation');this.leave(session);return{session,stepped:false,sent:false,reason:'simulation-error'};}
    try{
      const wire=this.sendPlayerSnapshot(session,snapshot);
      return{session,stepped:true,sent:wire!==null&&wire!==undefined,reason:wire===null||wire===undefined?'transport-unavailable':'snapshot-sent',snapshot};
  ...
  start(){
    ...
    this.timer=this.setIntervalFn(()=>{try{this.tickOnce();}catch(error){this.reportSessionError(null,error,'tick-loop');}},AUTHORITATIVE_WORLD_TICK_MS);return true;
  }
