...
- 被动和僵尸运行时现已使用 EntityStore / SpatialHash；DropSystem 仍未迁移，死亡掉落/经验和实体持久化仍未完成。
- 生物 AI 与渲染更新解耦：AI 固定 10 Hz，视觉按帧插值；当前 16 被动 + 8 敌对以及 48 格回收是浏览器阶段性能边界，不视为最终 Minecraft 规则。
- 僵尸通过 `onPlayerHit` 回调请求伤害，玩家伤害/死亡规则不反向耦合进敌对 AI 模块。

### Known limitations
- 当前僵尸只有直接追踪和近战，没有寻路、日照燃烧、视线/亮度生成规则、装备或门交互。
- 当前战斗只建立基础冷却/伤害/击退链，没有 Java 版完整攻击强度、暴击、扫击、护甲、附魔、死亡掉落和经验规则。
