...
- 被动和僵尸运行时使用 EntityStore / SpatialHash；DropSystem 与 ExperienceOrbSystem 当前仍保持受控小数组，不为了统一命名提前引入额外复杂度。
- 生物 AI 与渲染更新解耦：AI 固定 10 Hz，视觉按帧插值；当前 16 被动 + 8 敌对以及 48 格回收是浏览器阶段性能边界，不视为最终 Minecraft 规则。
- 僵尸通过 `onPlayerHit` 回调请求伤害，生物死亡通过 `onDeath` 回调请求奖励，AI 模块不直接修改 HUD、背包或经验状态。
- Loot/Xp roll 与经验等级公式都是可注入 RNG / 纯函数规则；运行时只负责把结果表现为掉落物和经验球。
- `totalXp` 是经验的单一持久化真相源，等级和条内进度不重复存档。

### Known limitations
- 当前僵尸只有直接追踪和近战，没有寻路、日照燃烧、视线/亮度生成规则、装备或门交互。
- 当前战斗只建立基础冷却/伤害/击退链，没有 Java 版完整攻击强度、暴击、扫击、护甲或附魔规则。
- 当前 loot 是简化静态表，没有 Looting、火焰击杀熟食、幼体差异、稀有掉落或完整条件化 loot table。
- 玩家死亡目前不会掉落背包，也不会丢失/散落已有经验；没有死亡界面和死亡统计。
- 生物实体、掉落物和经验球本身都尚未跨世界重载持久化。
- loot 内联 SVG 色块只是功能占位，不是正式 Minecraft 风格物品美术。
