#!/usr/bin/env python3
"""Build a deterministic texture atlas for interpreted Minecraft block models.

This builder consumes the source-archive dependency closure instead of a
hand-maintained texture list.  It deliberately emits a separate atlas from the
legacy 4x4 terrain atlas so the existing tile-ID contract remains untouched.

The first accepted runtime contract supports static square model textures only.
Animated metadata and non-square textures fail closed rather than silently
producing incorrect UVs.  Every packed image receives a one-pixel replicated
edge gutter to prevent neighbouring regions from bleeding during sampling.
"""
...
class AtlasError(RuntimeError):
    """Raised when the declared texture closure cannot be packed safely."""
...
def _decode_texture(index: MinecraftArchiveIndex, canonical: str) -> TextureSource:
    ...
    if index.has(metadata):
        raise AtlasError(
            ...
            "animation metadata must be implemented before this texture can be admitted"
        )
...
def _paste_with_gutter(atlas: Image.Image, placement: Placement) -> None:
    ...
    left = pixels.crop((0, 0, 1, height)).resize((GUTTER_PX, height), Image.Resampling.NEAREST)
    right = pixels.crop((width - 1, 0, width, height)).resize((GUTTER_PX, height), Image.Resampling.NEAREST)
    top = pixels.crop((0, 0, width, 1)).resize((width, GUTTER_PX), Image.Resampling.NEAREST)
    bottom = pixels.crop((0, height - 1, width, height)).resize((width, GUTTER_PX), Image.Resampling.NEAREST)

    atlas.paste(left, (placement.x, y))
    atlas.paste(right, (x + width, y))
    atlas.paste(top, (x, placement.y))
...
def build_model_texture_atlas(
    ...
    try:
        with ZipFile(archive_path) as archive:
            ...
            if closure.metadata:
                raise AtlasError(
                    "animated texture metadata entered the model-atlas closure: "
                    + ", ".join(closure.metadata)
                )
