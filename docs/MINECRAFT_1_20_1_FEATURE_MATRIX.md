...
Percentages are planning estimates, not automated coverage. `PROJECT_BASELINE.md` records merged main; this matrix describes the **projected post-PR #128 state** where explicitly noted.

## Overall projected state after PR #128
...
| Singleplayer survival core | 75% | PARTIAL | natural seed acquisition, food-use duration/effects, enchanting/brewing, broad progression |
| Blocks/items/recipes breadth | 26% | PARTIAL | most 1.20.1 registry content absent |
...
| Original resource integration | 52% | PARTIAL | broad registry use, tint/animation, generated sound registry |
| Audio / SFX / music | 12% | PARTIAL | current tool/block/mining/mob subset; broad events/spatial/music remain |
| Farming / food / processing | 56% | PARTIAL | first wheat loop exists; vegetation seeds, exact crop ticks, effects, broader crops/foods absent |
...
**Overall strict Java 1.20.1 parity remains conservatively about 35%.** The first wheat loop closes a meaningful survival path, but registry/worldgen/dimension/redstone/effects gaps still dominate the total.
...
| Generic blockstate/model interpreter | PARTIAL | Selected roots live, now including farmland/wheat; broad registry/state/collision breadth incomplete. |
...
| Hunger HUD | PARTIAL | Existing 10-shank HUD tracks food level; exact effect animation breadth remains. |

## Blocks / items / crafting

Projected post-#128 registry: **54 runtime item IDs / 19 crafting recipes** plus **5 Furnace recipes**.

Current gameplay families/states include:

`grass_block`, `dirt`, `stone`, `sand`, `oak_planks`, `oak_log`, `oak_leaves`, `water`, `crafting_table`, `cobblestone`, `red_bed`, `iron_ore`, `coal_ore`, `glass`, `furnace`, `farmland moisture 0..7`, `wheat age 0..7`, `dirt_path`, `stripped_oak_log`.
...
| Wheat seeds / wheat | PARTIAL | Canonical items, planting and harvest loop; natural short-grass seed source absent. |
| Wheat → bread | PARTIAL | 3-wide Workbench recipe is playable after harvest. |
...
Later iron/coal/food/farming additions are not inserted into historical `CREATIVE_START`; they remain registered/obtainable without shifting starter-slot contracts.
...
| Player-created farming states | PARTIAL | Sparse edits persist farmland moisture and wheat age IDs; terrain generator itself does not create crops. |
...
| Mob raw-food drops | PARTIAL | Existing meat drops enter the hunger loop; loot breadth remains simplified. |
...
| Furnace | PARTIAL | Authoritative process-memory state; recipes are shared pure rules; durable storage absent. |
...
| Farming | TODO | No server-owned planting/random tick/crop-drop transaction; client deliberately does not simulate multiplayer crops. |
...
| Farmland moisture / irrigation | PARTIAL | moisture 0..7, nearby water/rain hydration and drying implemented with simplified 10-second tick semantics. |
...
| Wheat growth | PARTIAL | age 0..7 and canonical models; simplified 0.45 wet / 0.20 dry growth chance rather than exact Java light/neighbor/random-tick formula. |
| Wheat harvest / seed recycling | PARTIAL | immature seed return and mature wheat + 0..3 seed loop; exact Fortune/RNG absent. |
| Bread acquisition | PARTIAL | harvested wheat can craft bread through the 3-wide Workbench recipe. |
| Natural seed acquisition | TODO | Short-grass vegetation/drop path absent. |
| Bone meal | TODO | Not implemented. |
...
## Immediate roadmap after #128

1. vegetation/farming 1.1: short grass, natural wheat-seed acquisition and bone-meal foundation;
