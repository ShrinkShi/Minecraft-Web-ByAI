...
| `src/mobs.js` | 被动/敌对生物静态规则、生成选择、loot/xp 表与可注入 RNG roll | 僵尸/骷髅/苦力怕攻击类型只存规则，不直接操作 HUD/存档 |
| `src/passive-mobs.js` | 被动生物生成、10 Hz 漫游/逃跑、命中、伤害/击退、死亡事件和视觉回收 | 使用 EntityStore/SpatialHash；死亡通过 callback 解耦奖励层 |
| `src/hostile-mobs.js` | 僵尸近战、骷髅远程/侧移、苦力怕 fuse、夜间生成、伤害与死亡事件 | 使用 EntityStore/SpatialHash；通过 callback 请求玩家伤害/投射物/爆炸/奖励 |
...
| `scripts/check.mjs` | Inventory/Recipes/Commands/Entity/Combat/Projectile/Mob/Loot/Experience/Workers 回归检查 | Node 22，无浏览器依赖 |
