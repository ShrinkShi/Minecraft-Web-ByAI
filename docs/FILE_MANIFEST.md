...
| `src/main.js` | 应用状态机、Three.js 场景、系统编排、交互、奖励与自动保存 | 不执行区块生成和网格重计算重活 |
...
| `src/mobs.js` | 被动/敌对生物静态规则、夜间判定、loot/xp 表与可注入 RNG 的奖励 roll | 只保存可测试规则，不保存运行时实体状态 |
| `src/passive-mobs.js` | 被动生物生成、10 Hz 漫游/逃跑、命中、伤害/击退、死亡事件和视觉回收 | 使用 EntityStore/SpatialHash；死亡通过 callback 解耦奖励层 |
| `src/hostile-mobs.js` | 僵尸夜间生成、10 Hz 追击/近战、命中、伤害/击退、死亡事件与视觉生命周期 | 使用 EntityStore/SpatialHash；通过 callback 请求玩家伤害/奖励 |
...
| `scripts/check.mjs` | Inventory/Recipes/Commands/Entity/Combat/Mob/Loot/Experience/Workers 回归检查 | Node 22，无外部测试依赖 |
