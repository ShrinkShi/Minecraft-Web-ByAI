...
状态：开发中。实体基础、第一批被动生物、僵尸近战和“击杀 → 战利品/经验球 → 拾取 → 等级/存档”奖励闭环已经接入；其余敌对 AI、护甲、实体持久化和完整死亡规则仍未完成。
...
- [x] 被动生物和僵尸死亡事件统一回调到奖励编排层
- [x] 第一批战利品：生牛肉/皮革、羊毛/生羊肉、生猪排、生鸡肉/羽毛、腐肉
- [x] `ExperienceOrbSystem`：经验球物理、吸附、拾取和 300 秒销毁
- [x] Java 风格经验等级阈值/总经验公式；覆盖 16/17、31/32 级边界的 Node 回归测试
- [x] `totalXp` 写入世界快照 v4，并恢复经验等级/HUD
- [x] Loot min/max 与未知实体奖励的确定性 RNG 回归测试
- [ ] 将 DropSystem / ExperienceOrbSystem 迁移或统一到 EntityStore / SpatialHash（当前数量受控，先不制造无收益复杂度）
- [ ] 被动生物繁殖和跨存档实体持久化
