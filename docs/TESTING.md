...
- Mob rules：牛/羊/猪/鸡选择、僵尸夜间窗口、loot min/max、零掉落、未知实体奖励和 XP roll。
...
- 被动生物/僵尸视觉、raycast 命中、10 Hz AI 与实际地形上的移动表现。
...
因此，Repository quality 成功只意味着“模块语法 + 当前纯逻辑/Worker 回归通过”，不等价于完整游戏运行无缺陷。后续一旦浏览器 E2E 基础设施可用，应优先补世界创建→击杀实体→掉落/经验拾取→保存→重载这一条关键链路。
