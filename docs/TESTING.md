...
- Crafting：原木→木板、2×2 工作台、3×3 木镐；死亡路径可抽空未合成输入。
...
`scripts/serve.mjs` 是本地/CI 共用的跨平台静态服务器。当前 smoke 验证：主菜单、世界创建、HUD、WebGL Canvas、暂停、IndexedDB 世界记录，以及未捕获页面/console error。
...
browser smoke 只证明核心页面可启动并能创建/保存世界，不等价于完整玩法无缺陷。仍需覆盖：

- Pointer Lock、F5 三视角和持续移动。
- 不同 GPU/浏览器驱动的 WebGL 材质/纹理兼容性。
- 被动生物与四种敌对生物在真实地形上的视觉/AI表现。
- 骷髅箭矢、苦力怕爆炸、蜘蛛追击的完整战斗链。
- 生存死亡→背包/cursor/合成格清算→死亡点物品/经验生成→重生，以及虚空死亡不生成实体的真实浏览器链。
- 掉落物和经验球大量存在时的性能。
