...
- `sleep-rules.js` 独立负责晴天/雨天睡眠窗口、雷暴例外、清晨目标和 sleeper percentage/quorum；它不读取 DOM、设备类型或 World。`activateBed()` 先走共享 `setRespawnPoint()`，再调用 `resolveSleep()`；桌面/手机都从同一个 secondary/use 动作进入。当前单人传 1/1；未来 server-authoritative 联机由服务器提供 sleepingPlayers/totalPlayers/percentage，设备平台不得影响 quorum。
