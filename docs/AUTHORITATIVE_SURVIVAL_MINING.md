...
`MiningCrackOverlay` 同样只订阅这条已经验证的多人 progress 展示通道。它把 0..1 的服务器 progress 映射为 10 个视觉阶段，在目标方块外表面绘制透明裂纹覆盖层；不会启动本地计时器，也不会根据帧率推进阶段。服务器 target 变化时覆盖层移动到新目标，progress reset、完成破坏或断开多人 gameplay 时立即隐藏并释放资源。

裂纹纹理由客户端程序化生成，因此不增加网络负载；网络只传递权威进度与目标。这里的“本地渲染”只表示像素由客户端画出，不表示客户端拥有挖掘进度的决定权。
...
当前裂纹覆盖只表示本地玩家正在服务器上挖掘的目标。远端玩家手臂挥动、远端玩家挖掘目标提示等仍需独立的多人动作复制设计；不能通过观察 world change 或本地猜测来伪造。
